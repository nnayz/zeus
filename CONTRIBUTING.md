...
- Rust — the toolchain is pinned in `zeus/rust-toolchain.toml` and rustup will
  fetch it for you
...
zeus is one app made of two codebases, and which one you touch depends on what
you are changing:

- **`zeus/`** — the Rust + GPUI desktop app. Window, sidebar, terminal
  rendering, command palette, usage accounting.
- **`Sources/`** — the Swift engine. `zeusd` owns the PTYs and child agent
  processes so sessions outlive the app; `zeus` is the CLI and MCP shim.
...
(cd zeus && cargo build && cargo test)   # app
```
...
```sh
swift test
(cd zeus && cargo fmt --all -- --check)
(cd zeus && cargo clippy --workspace --all-targets -- -D warnings)
(cd zeus && cargo test --workspace)
```

To try your change in the real app, `zeus/scripts/package.sh` builds the bundle
and `zeus/scripts/install-local.sh` installs it to `~/Applications`.
...
- They are wall-clock sensitive. `ZEUS_TEST_TIMEOUT_SCALE` multiplies every
...
- Browser tests are opt-in behind `ZEUS_RUN_BROWSER_TESTS=1` and need
...
- Two tests are skipped on CI because they hang there (#1). They still run
  ...
  `Hang repro` workflow: it sets `ZEUS_RUN_HANGING_TESTS=1` and runs
...
data: each agent is one JSON file in `Sources/ZeusCore/Resources/manifests/`
