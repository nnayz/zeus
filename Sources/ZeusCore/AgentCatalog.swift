...
/// The set of agents Zeus knows how to run, loaded from manifest files.
///
...
/// (which `ZeusDetection.ManifestEngine` reads from the same files). Both
...
public struct AgentCatalog: Sendable {
    ...
    /// Gives menus and `zeus agents` a stable, non-random ordering.
    ...
    /// `~/Library/Application Support/Zeus/manifests/overrides`.
    ///
    /// Duplicated from `ZeusPaths` on purpose: ZeusProtocol depends on
    /// ZeusCore, so Core cannot import it back. Keep the two in sync.
    public static var overridesDirectory: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Zeus/manifests/overrides", isDirectory: true)
    }

    /// Manifest files in load order: bundled first, then user overrides (which
    /// replace a bundled agent of the same id). Also what ZeusDetection
...
/// Manifest `id` values Zeus itself has behavior for. Everything else is
