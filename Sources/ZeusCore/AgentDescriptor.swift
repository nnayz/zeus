...
/// Everything Zeus needs to know about ONE agent CLI, declared as data.
///
/// This is the `"agent"` block of a detection manifest
/// (`Sources/ZeusCore/Resources/manifests/<id>.json`). It exists so that
...
public struct AgentDescriptor: Codable, Hashable, Sendable {
    ...
    /// `StatusReducer.Authority`, which lives in ZeusDetection and therefore
    ...
    /// Per-launch config injection. Each flag names a *mechanism* Zeus
    ...
    /// Lowercase one-word label for compact output (MCP results, `zeus ls`).
    ...
    /// True when Zeus has real status detection for the agent (a manifest
    ...
    /// (`--session-id`). When set, Zeus generates the UUID at spawn so it
    ...
    /// environments so a Zeus started *inside* an agent doesn't hand its
    ...
    /// Whether Zeus can actually resume this agent's conversations.
    ///
