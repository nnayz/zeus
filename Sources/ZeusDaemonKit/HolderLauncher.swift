...
enum HolderLauncher {
    ...
    static func defaultExecutablePath() -> String {
        if let configured = ProcessInfo.processInfo.environment["ZEUS_HOLDER_PATH"],
        ...
        let candidates = [
            executable.deletingLastPathComponent().appendingPathComponent("zeusd-holder"),
            Bundle.main.executableURL?.deletingLastPathComponent()
                .appendingPathComponent("zeusd-holder"),
            URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
                .appendingPathComponent(".build/debug/zeusd-holder"),
        ].compactMap { $0 }
