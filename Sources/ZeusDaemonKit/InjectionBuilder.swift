...
public enum InjectionBuilder {
    ...
    /// exit also tears down the PTY and Zeus can only show "agent exited".
    ///
    ...
    static func baseEnvironment(
        ...
        var env: [String: String] = [
            ZeusEnv.sessionID: sessionID.rawValue,
            ZeusEnv.socket: socketPath,
            ZeusEnv.cli: cliPath,
    ...
    /// opted into. Each is a Zeus-implemented shim (hooks file, MCP server
    ...
    public static func resumeArgv(
        ...
        // loses status detection and the zeus MCP tools.
        //
    ...
    /// tmux session name on the remote host, derived from the persisted Zeus session
    ...
    public static func remoteTmuxSessionName(sessionID: SessionID) -> String {
        ...
        return "zeus-" + String(raw.prefix(8))
    }
    ...
    /// scrubbing "CURSOR" would kill CURSOR_API_KEY. "ZEUS" is ours, not any
    /// agent's, so it stays here.
    static let agentNestingEnvPrefixes: [String] =
        AgentCatalog.shared.envScrubPrefixes + ["ZEUS"]

    /// Strips agent-nesting variables from an inherited environment. Zeus's
    /// own variables (ZEUS_*, CLAUDE_CODE_NO_FLICKER) are re-applied per
    ...
    public static func writeCursorPlugin(
        ...
        if mcp {
            ...
            let mcpConfig: [String: Any] = ["mcpServers": ["zeus": server]]
        ...
        if hooks {
            ...
            // Absolute CLI path: Cursor does not expand $ZEUS_CLI in hook
    ...
    // MARK: Zeus MCP injection (agents can spawn/orchestrate other agents)

    /// `--mcp-config` args pointing at the daemon-written server file, so every
    /// spawned/resumed Claude has the `zeus` MCP tools (spawn_agent,
    ...
    static func codexMcpArgs(cliPath: String) -> [String] {
        ...
        return [
            "-c", "mcp_servers.zeus.command=\(tomlString(launch.command))",
            "-c", "mcp_servers.zeus.args=[\(args)]",
        ]
    }

    /// Writes the Claude `--mcp-config` file: the `zeus` stdio server backed
    ...
    private static func mcpLaunch(cliPath: String) -> (command: String, args: [String]) {
        let proxy = URL(fileURLWithPath: cliPath)
            ...
            .appendingPathComponent("zeus-mcp").path
    ...
    public static func writeClaudeHooksFile(into injectDir: URL) throws {
            ...
            hooks[event] = .array([
                .object([
                    "hooks": .array([
                        .object([
                            "type": .string("command"),
                            "command": .string("\"$\(ZeusEnv.cli)\" hook \(event)"),
                            "timeout": .number(10),
                        ])
                    ])
                ])
        ...
        let data = try JSONEncoder.zeus.encode(root)
