...
public struct DaemonConfig: Sendable {
    ...
    /// (the UDS-only default). Loaded from `ZeusPaths.remoteConfigFile`.
    ...
    public init(
        ...
        self.remote = remote
        self.hostsConfigFile = hostsConfigFile ?? ZeusPaths.hostsConfigFile
    }

    public static func standard() -> DaemonConfig {
        DaemonConfig(
            socketPath: ZeusPaths.socket.path,
            cliPath: ZeusPaths.binDir.appendingPathComponent("zeus").path,
            injectDir: ZeusPaths.injectDir,
            logsDir: ZeusPaths.logsDir,
            stateFile: ZeusPaths.stateFile,
            holdersDir: ZeusPaths.holdersDir,
            remote: RemoteConfig.load(from: ZeusPaths.remoteConfigFile),
            hostsConfigFile: ZeusPaths.hostsConfigFile
        )
    }
}
...
public actor SessionRegistry {
    ...
    public func spawn(_ params: SessionSpawnParams) async throws -> SessionRecord {
        ...
        var record = SessionRecord(
            ...
            titleSource: params.title != nil ? .zeusAssigned : .placeholder,
    ...
    /// SAME zeus id (the tmux promise after reboot).
