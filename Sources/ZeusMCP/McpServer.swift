...
public final class McpServer: @unchecked Sendable {
    ...
    private func processLine(_ line: Data) async {
        ...
        do {
            message = try JSONDecoder.zeus.decode(JSONValue.self, from: Data(trimmed))
        } catch {
    ...
    private func writeMessage(_ value: JSONValue) {
        guard var data = try? JSONEncoder.zeus.encode(value) else { return }
        data.append(0x0A)
    ...
    private func initializeResult(params: JSONValue) -> JSONValue {
        ...
        let browserInstructions =
            ProcessInfo.processInfo.environment["ZEUS_TEST_RUN_AVAILABLE"] == "1"
            ? """

                To TEST a web feature, use test_run — it drives real browser
                engines through Zeus's shared Playwright pool. Feed it a preview
        ...
        return .object([
            ...
            "instructions": .string(
                """
                This session is running INSIDE Zeus, a macOS orchestrator for \
    ...
    static func toolTextContent(_ value: JSONValue, isError: Bool) -> JSONValue {
        ...
        if case .string(let s) = value {
            text = s
        } else if let data = try? JSONEncoder.zeus.encode(value),
