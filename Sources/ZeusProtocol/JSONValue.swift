...
public enum JSONValue: Codable, Hashable, Sendable {
    ...
    public init<T: Encodable>(encoding value: T) throws {
        let data = try JSONEncoder.zeus.encode(value)
        self = try JSONDecoder.zeus.decode(JSONValue.self, from: data)
    }

    /// Decodes this JSONValue into a concrete Decodable.
    public func decoded<T: Decodable>(as type: T.Type = T.self) throws -> T {
        let data = try JSONEncoder.zeus.encode(self)
        return try JSONDecoder.zeus.decode(T.self, from: data)
    }
}

extension JSONEncoder {
    /// Canonical encoder for all Zeus wire traffic and persisted state.
    public static var zeus: JSONEncoder {
...
extension JSONDecoder {
    public static var zeus: JSONDecoder {
