...
enum CLIClient {
    ...
    static func connect(socket: String? = nil) throws -> DaemonConn {
        do {
            ...
            FileHandle.standardError.write(Data("zeus: daemon unreachable (\(error))\n".utf8))
    ...
    static func translate(_ error: DaemonError) -> Error {
        switch error {
        case .timeout:
            FileHandle.standardError.write(Data("zeus: timed out\n".utf8))
            return CLIExit.timeout
        case .control(let control):
            FileHandle.standardError.write(
                Data("zeus: \(control.code): \(control.message)\n".utf8))
    ...
    static func resolve(_ needle: String, in sessions: [SessionRecord]) throws -> SessionRecord {
        ...
        if candidates.count > 1 {
            ...
            FileHandle.standardError.write(
                Data("zeus: \"\(needle)\" matches \(candidates.count) sessions: \(ids)\n".utf8))
            throw CLIExit.failure
        }
        FileHandle.standardError.write(Data("zeus: no such session: \(needle)\n".utf8))
...
extension AgentKind {
    ...
    /// which is what makes `zeus session spawn htop` work.
    ///
