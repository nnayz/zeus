...
final class DaemonConn {
    ...
    /// The control socket path from `$ZEUS_SOCKET`, else the default location.
    static func socketPath() -> String {
        if let env = ProcessInfo.processInfo.environment[ZeusEnv.socket], !env.isEmpty {
            return env
        }
        return ZeusPaths.socket.path
    }
