...
/// `zeus events subscribe|wait` — the piece that turns the daemon from
    ...
    func run() throws {
        ...
        do {
            try conn.stream(Method.eventsSubscribe, params: params) { name, seq, params in
                ...
                // makes `zeus events subscribe | head -5` terminate promptly
...
struct EventsWait: ParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        discussion: """
            ...
            `zeus events wait --until needs-input --timeout 60` is a usable \
            shell condition.
            """)
