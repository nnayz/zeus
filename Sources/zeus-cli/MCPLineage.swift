...
struct SessionLineage {
    ...
    /// The session this MCP server is running inside, from `ZEUS_SESSION_ID`.
    /// nil when the tools are driven from outside Zeus (a plain shell, a
    ...
    func frame(_ text: String, relation: Relation) -> String {
        ...
        return "[message from \(who), channel: zeus — reply with send_prompt to that id]\n\n\(text)"
    }
