...
extension MCPBridge {
    ...
    static func requireCaller(_ lineage: SessionLineage) throws -> SessionID {
        guard let caller = lineage.caller else {
            throw BridgeError(
                message:
                    "This tool needs to know which session is calling it, and ZEUS_SESSION_ID is unset — run it from a session hosted by Zeus."
            )
        }
        return caller
    }
    ...
    static func whoami(_ args: JSONValue) throws -> JSONValue {
        ...
        guard let caller = lineage.caller else {
            return .object([
                "hosted": .bool(false),
                "note": .string(
                    "Not running inside a Zeus session (ZEUS_SESSION_ID is unset). Lineage tools are unavailable and writes to other sessions are unrestricted and unattributed."
                ),
            ])
        }
        guard let record = lineage.record(caller) else {
            throw BridgeError(
                message:
                    "ZEUS_SESSION_ID is \(caller.rawValue) but the daemon has no such session — the record may have been removed."
            )
        }
