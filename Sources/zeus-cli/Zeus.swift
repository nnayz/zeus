...
@main
struct Zeus: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "zeus",
        abstract: "Zeus agent-orchestrator CLI.",
        subcommands: [
...
struct Hook: ParsableCommand {
    ...
    private func performHook() throws -> String {
        ...
        let params = HookReportParams(
            kind: "claude-hook",
            zeusSessionID: CLISupport.sessionID(),
            event: event,
            payload: payload
        )
        ...
        // SessionStart may carry a zeus-assigned title to inject.
...
struct McpStdio: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        ...
        abstract: "Run the Zeus MCP server on stdio, proxying tools to the daemon."
    )

    func run() async {
        Self.execStandaloneProxyIfAvailable()
        let server = McpServer(tools: ZeusMCPTools.all) { name, arguments in
    ...
    /// Old agent configurations launch `zeus mcp-stdio`. Preserve that argv
...
struct McpTools: ParsableCommand {
    ...
    func run() {
        print(CLISupport.encodeCompact(ZeusMCPTools.listResult))
    }
}

/// One-shot tool backend for `zeus-mcp`. The Swift runtime is paid only
    ...
    func run() throws {
        ...
        if FileManager.default.fileExists(atPath: socketPath) {
            do {
                let conn = try DaemonConn.connect()
                defer { conn.close() }
                let params = HelloParams(build: "zeus-cli/\(McpServer.serverVersion)")
        ...
        // 3. State file.
        let statePath = ZeusPaths.stateFile.path
