...
final class BrowserPoolTests: XCTestCase {
    func testRunAcrossEnginesReportsPass() async throws {
        guard ProcessInfo.processInfo.environment["ZEUS_RUN_BROWSER_TESTS"] == "1" else {
            throw XCTSkip("set ZEUS_RUN_BROWSER_TESTS=1 (needs node + playwright browsers)")
        }
        ...
        let steps: [JSONValue] = [
            .object(["fill": .array([.string("#name"), .string("Zeus")])]),
            .object(["click": .string("#go")]),
            .object(["assert": .object(["selector": .string("#out"), "text": .string("Hi Zeus")])]),
        ]
    ...
    func testInteractiveBrowserDrivesPageByRef() async throws {
        ...
        try await withPool(tmp) { pool in
            ...
            let consoleText = String(
                data: (try? JSONEncoder.zeus.encode(console)) ?? Data(), encoding: .utf8) ?? ""
            XCTAssertTrue(
