...
//   ZEUS_REPLAY_FILE=/path/bytes.bin ZEUS_REPLAY_SIZE=220x66 \
//   ZEUS_REPLAY_START=120x32 ZEUS_REPLAY_RESIZE_AT=2048 \
...
@Test func replayCapturedLog() throws {
    guard let path = ProcessInfo.processInfo.environment["ZEUS_REPLAY_FILE"] else { return }
    ...
    let final = parseSize(env["ZEUS_REPLAY_SIZE"]) ?? (220, 66)
    let start = parseSize(env["ZEUS_REPLAY_START"]) ?? final
    let resizeAt = Int(env["ZEUS_REPLAY_RESIZE_AT"] ?? "") ?? 0
