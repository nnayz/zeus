...
@Test func claudeSpawnPlanPreGeneratesSessionID() {
    ...
    #expect(plan.extraEnv[ZeusEnv.sessionID] == "s_abc")
    #expect(plan.extraEnv[ZeusEnv.socket] == "/tmp/d.sock")
}
...
@Test func codexSpawnPlanInjectsNotify() {
    let plan = InjectionBuilder.plan(
        ...
        cwd: "/tmp",
        socketPath: "/tmp/d.sock",
        cliPath: "/Applications/My App/zeus",
        injectDir: URL(fileURLWithPath: "/nonexistent")
    )
    ...
    // entire Zeus session. Resolving the bare `codex` inside that fresh shell
...
/// `CI=` (empty) to force it on locally, or `ZEUS_RUN_HANGING_TESTS=1` to
...
func codexWrapperReentersShellAndResolvesFreshInteractivePath() throws {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("zeus-codex-wrapper-\(UUID().uuidString)")
...
@Test func sanitizeInheritedEnvironmentDropsAgentNestingVars() {
    ...
    #expect(clean.keys.filter { $0.hasPrefix("ZEUS") }.isEmpty)
...
@Test func claudeHooksFileIsValidAndEnvBased() throws {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("zeus-inject-\(UUID().uuidString)")
    ...
    let json = try JSONDecoder.zeus.decode(JSONValue.self, from: data)
    ...
    #expect(encoded.contains("$ZEUS_CLI"))
}

@Test func claudeMcpConfigPrefersTheStandaloneProxy() throws {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("zeus-mcp-inject-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: dir) }
    let cli = dir.appendingPathComponent("zeus")
    let proxy = dir.appendingPathComponent("zeus-mcp")
    ...
    let config = servers["zeus"] as! [String: Any]
...
@Test func cursorPluginIsLaunchScopedWithBakedEnvAndStopHook() throws {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("zeus-cursor-plugin-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: dir) }
    let cli = dir.appendingPathComponent("Application Support/zeus")
    ...
    let proxy = cli.deletingLastPathComponent().appendingPathComponent("zeus-mcp")
    ...
    let server = ((mcp["mcpServers"] as! [String: Any])["zeus"] as! [String: Any])
    #expect(server["command"] as? String == "/usr/bin/env")
    #expect((server["args"] as? [String]) == [proxy.path])
    let env = server["env"] as! [String: String]
    #expect(env[ZeusEnv.sessionID] == "s_cursor")
    #expect(env[ZeusEnv.socket] == "/tmp/d.sock")
    #expect(env[ZeusEnv.cli] == cli.path)
...
@Test func cursorPluginRewriteDropsDisabledComponents() throws {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("zeus-cursor-plugin-reuse-\(UUID().uuidString)")
    try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: dir) }
    let cli = dir.appendingPathComponent("bin/zeus")
    try FileManager.default.createDirectory(
        at: cli.deletingLastPathComponent(), withIntermediateDirectories: true)
    FileManager.default.createFile(atPath: cli.path, contents: Data("#!/bin/sh\n".utf8))
    try FileManager.default.setAttributes([.posixPermissions: 0o755], ofItemAtPath: cli.path)
...
@Test func findClaudeTranscriptScansAllProjectDirs() throws {
    let projects = FileManager.default.temporaryDirectory
        .appendingPathComponent("zeus-test-projects-\(UUID().uuidString)")
