...
@Test func forwardPipelinedHandshakePayloadReachesTarget() async throws {
    ...
    var payload = try JSONEncoder.zeus.encode(request)
    payload.append(0x0A)
...
@Test func forwardProtocolDisambiguatesAttachAndForward() throws {
    ...
    #expect((try? JSONDecoder.zeus.decode(AttachRequest.self, from: attachLine)) != nil)
    #expect((try? JSONDecoder.zeus.decode(ForwardRequest.self, from: attachLine)) == nil)
    #expect((try? JSONDecoder.zeus.decode(ForwardRequest.self, from: forwardLine)) != nil)
    #expect((try? JSONDecoder.zeus.decode(AttachRequest.self, from: forwardLine)) == nil)
}

@Test func remoteConfigDecodesLegacyWithoutForwardAnyPort() throws {
    let data = Data(#"{"port":48620,"token":"t"}"#.utf8)
    let config = try JSONDecoder.zeus.decode(RemoteConfig.self, from: data)
...
private final class RawForwardClient {
    ...
    func sendForward(port: UInt16, token: String?) throws {
        ...
        var line = try JSONEncoder.zeus.encode(request)
        line.append(0x0A)
        try write(line)
    }
    ...
    func readAck(timeout seconds: Int) throws -> ForwardAck? {
        ...
        return try JSONDecoder.zeus.decode(ForwardAck.self, from: line)
    }
