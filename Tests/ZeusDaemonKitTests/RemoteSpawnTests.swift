...
@Test func remoteTmuxSessionNameDerivesFromSessionID() {
    #expect(
        InjectionBuilder.remoteTmuxSessionName(
            sessionID: SessionID(rawValue: "s_9f8e7d6c5b4a")) == "zeus-9f8e7d6c")
    // Ids without the s_ prefix still yield a stable 8-char suffix.
    #expect(
        InjectionBuilder.remoteTmuxSessionName(
            sessionID: SessionID(rawValue: "abcdef1234")) == "zeus-abcdef12")
}
...
@Test func remoteClaudeArgvIsSshTmuxWithPlainAgentCommand() {
    ...
    #expect(
        argv == [
            ...
            "-o", "TCPKeepAlive=yes",
            "cristi@forge", "--",
            "tmux", "new-session", "-A", "-s", "zeus-9f8e7d6c",
            "-c", "~/'code'",
...
@Test func remotePlanMintsAgentIDAndSkipsLocalTranscript() {
    ...
    #expect(plan.extraEnv[ZeusEnv.sessionID] == "s_9f8e7d6c5b4a")
    ...
    #expect(revived.argv.contains("zeus-9f8e7d6c"))
}
...
@Test func registryRejectsWorktreeAndUnknownHostSpawns() async throws {
    let dir = FileManager.default.temporaryDirectory
        .appendingPathComponent("zeus-remote-\(UUID().uuidString)")
