...
description: Install Zeus, launch your first agent session, and keep work running after you quit.
---
...
and notarized release ships. See [Updates](/docs/updates).
...
in the sidebar and in the workflow tree. See [Orchestration](/docs/orchestration).
...
- **SSH hosts** live under Settings → Remote. Zeus bootstraps a small Helper
  on the machine. No `tmux`. See [Remote hosts](/docs/remote-hosts).
- **First-party nodes** add accounts, usage, and laptop↔VPS handoff. See
  [Remote nodes](/docs/remote-nodes).
...
not connect to the Engine or to SSH directly. See [iOS Companion](/docs/ios-companion)
...
**[hi@nasrul.info](mailto:hi@nasrul.info)** for help. See [Support](/docs/support)
for what to include.
...
Read [Privacy](/docs/privacy) and the [security model](/docs/security-model) before
