...
Zeus has no account system, advertising, analytics, or telemetry. There is no
Zeus-operated service that receives your terminal contents or session history.
...
printed by a process. Treat them as sensitive and redact them before sharing
diagnostics. Archiving can intentionally preserve session metadata. Deleting the
directories above removes all Zeus-managed local data after Zeus and its daemon
are stopped.
...
SSH configuration and your keychain); they are not sent to Zeus.
...
For vulnerability reports, email **[hi@nasrul.info](mailto:hi@nasrul.info)**
