...
| Exact execution authorization is possible | Exact opaque Holder/child birth tokens, pinned adoption, in-memory consent generations/nonces/deadlines, and consent-bound lease tests | **LOCAL/MOCK TESTED; ENGINE ACTION NOT INTEGRATED** | Authenticated desktop action, Engine restart/upgrade and physical fixtures |
| Journal recovery is deterministic | Pure crash-point machine plus exclusive kernel lifetime lock and bounded checksummed same-directory atomic journal fixtures; ambiguous `Prepared + Disabled` remains blocked | **FILESYSTEM/MOCK TESTED; PRIVILEGED PLATFORM NOT TESTED** | Root-path, disk-full/torn-write, signed lifecycle and physical failure evidence |
...
| ServiceManagement identity/lifecycle works | Inert universal Helper and fixed demand-only launchd metadata only; no registration call | **SCAFFOLD ONLY; PHYSICAL TEST NOT RUN** | Authenticated IPC plus signed install/update/rollback/uninstall lab report |
