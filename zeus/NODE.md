...
`zeus-node` makes a VPS (or another workstation) a first-party Zeus execution host. It is a per-user service: provider credentials remain on the machine where Claude Code or Codex runs, while Zeus gets one versioned management interface for accounts, usage, provider sessions, and handoff.
...
The node config is owner-only and contains its app-layer capability token. Tailscale encrypts the transport; the token authenticates Zeus at the application layer. Do not bind the listener to a public interface and do not copy `node.json` wholesale.
...
In zeus, open Settings → Remote and add both the SSH fallback and first-party node fields. The resulting `hosts.json` entry is:
