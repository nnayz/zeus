...
`scripts/package.sh` builds `zeus` for Apple silicon and Intel, combines the two slices with `lipo`, asks cargo-packager to assemble `dist/zeus.app`, and signs the result. The bundle identifier is `com.zeus.zeus`, the deployment target is macOS 15.0, and the app does not use App Sandbox.
...
```sh
export ZEUS_SIGN_IDENTITY='Developer ID Application: Example Team (TEAMID)'
export ZEUS_CREATE_DMG=1
export APPLE_NOTARIZATION_KEYCHAIN_PROFILE=zeus-notary
scripts/package.sh
```
...
Notarizing also produces `dist/zeus-<version>-universal.zip`, the artifact zeus's updater downloads. `ZEUS_CREATE_ZIP=1` builds it without notarization too. The version in both artifact names comes from `crates/zeus-app/Cargo.toml` unless `ZEUS_VERSION` overrides it, because the updater compares against `CARGO_PKG_VERSION`.
