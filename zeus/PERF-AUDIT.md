...
moot if the zeusd-rs flip sticks), C6, C8, C9.
...
## A. GPUI client (`zeus-app`, `zeus-term`)
...
`EventBus.archive`; `JSONEncoder.zeus`/`JSONDecoder.zeus` are computed
...
### C6. zeus-node re-parses whole transcripts from byte 0 every 30s
