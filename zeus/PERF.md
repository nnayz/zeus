...
```sh
export PATH=/tmp/zeus-cargo-home/bin:$PATH
export CARGO_HOME=/tmp/zeus-cargo-home
export RUSTUP_HOME=/tmp/zeus-rustup-home
export CARGO_TARGET_DIR=/tmp/zeus-shared-target
cargo build -p zeus-app --release
```
...
`ZEUS_PERF_LARGE_WINDOW=1` is a retained profiling switch that starts the app at
...
`scripts/perf-gate.sh` launches the executable inside `dist/zeus.app` directly
...
It does not use `pkill`, name-based termination, or touch an existing Zeus. On
...
```sh
zeus/scripts/perf-gate.sh --app zeus/dist/zeus.app --scenario all
```

Budgets are configurable for deliberate tightening:

```sh
ZEUS_PERF_NORMAL_MAX_MB=80 \
ZEUS_PERF_LARGE_MAX_MB=140 \
ZEUS_PERF_IDLE_AVG_CPU=0.75 \
ZEUS_PERF_IDLE_PEAK_CPU=1 \
  zeus/scripts/perf-gate.sh --app zeus/dist/zeus.app
```

`zeus/scripts/release.sh` runs this after the final bundle is signed, notarized,
