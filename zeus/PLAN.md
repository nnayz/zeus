# zeus — GPUI Migration Plan

A ground-up rewrite of the Zeus **client app** in Rust + [GPUI](https://gpui.rs) (Zed's GPU-accelerated UI framework). Same architecture, same design, same daemon — a new binary and a new `.app` named **zeus** that coexists with the shipping Zeus without touching it.
...
- Zero interference with the running Zeus installation and its live agent sessions.
- New app identity: binary `zeus`, bundle `zeus.app`, its own preferences domain.

**Non-goals (v1)**
- No daemon rewrite. `zeusd` (Swift) stays exactly as-is; zeus is a pure client.
...
Zeus already uses a **mosh/tmux model** (verified in source):

- The **daemon owns everything stateful**: PTYs, VT emulation (SwiftTerm headless, `ZeusDaemonKit/HeadlessScreen.swift`), scrollback (2000 lines), status detection, persistence, hibernation, injection, worktrees, resource governor, MCP.
- Clients receive a **rendered grid of cells** — RLE-encoded row diffs (`ZeusProtocol/Grid.swift`) — over a unix socket, and send back input bytes, resize, and scroll.
- The SwiftUI app performs **no VT parsing at all**. `GridTerminalView` is a dumb cell painter.

Therefore zeus is: **a protocol client + a GPU cell renderer + the app shell.** No `alacritty_terminal`, no PTY code, no emulator. This is why the port is tractable and why zeus inherits every session (including MCP-spawned ones) for free — they arrive as `session.updated` events from the shared daemon.

**Reference reading order for any task agent:**
1. `Sources/ZeusProtocol/{Frames,Grid,ControlMessage,Methods,Paths}.swift` — the wire format (port spec).
2. `Sources/ZeusClient/{DaemonClient,SessionAttachment}.swift` — the client behavior spec.
3. `Sources/ZeusTerminalHost/GridTerminalView*.swift` + `TerminalKeyEncoding.swift` — renderer + input spec.
4. `Sources/ZeusCore/*.swift` — the data model (`SessionRecord`, `SessionStatus`, `AgentKind`, `AttentionLevel`, …).
5. `Sources/ZeusMac/*` — UI spec. Treat `ZeusDaemonKit` as a black box.

> Sources 3 and 5 were the Swift client, removed once the port landed. The
> behavior they specified now lives in `crates/zeus-term`, `crates/zeus-ui`,
> and `crates/zeus-app`.
...
1. **Never ship or spawn a different `zeusd`.** The Swift app compares `hello.executableHash` against its bundled daemon and restarts on mismatch (`AppModel.swift:655`). If zeus did the same with a different binary, the two apps would ping-pong-restart the daemon forever. zeus therefore:
   - **Does not bundle a daemon.** It connects to the existing socket (`~/Library/Application Support/Zeus/daemon.sock`).
   - **Never performs the stale-daemon restart dance.** The sole user-initiated exception is Settings → Remote: after changing `remote.json`, zeus calls `daemon.shutdown` once so the listener reloads; holder-owned sessions survive and zeus relaunches only if launchd has not already done so.
   - If the socket is dead, zeus shows a "Zeus daemon not running" state with a button that launches the installed Zeus daemon binary (resolve via the installed app bundle) — or simply tells the user to open Zeus. v1: connect-only + spawn the installed `zeusd` if found (same singleton `flock` makes double-spawn safe).
2. **Geometry arbitration is desktop+mobile, not desktop+desktop.** One PTY per session, one effective size. Two `.desktop` attachments with different window sizes will fight over resize (SIGWINCH thrash). Mitigation for v1: zeus attaches as `role: "desktop"` like the Swift app, which is fine as long as the *same session* isn't focused in both apps at once (Zeus only keeps 3 resident attachments — MRU). Document this in zeus's README; a proper multi-desktop role is a later additive protocol change. Do **not** attach as `mobile` — that flips `remoteActive` and Zeus shows an "Active on iPhone" takeover card.
...
4. **Wire compatibility:** `hello` with `proto: 1` (`WireVersion.current`), `build: "zeus-<semver>"`, `token: null` on local UDS. Protocol is additive-only within major 1 — decode leniently (unknown fields ignored, unknown enum values mapped to a fallback).
5. **Separate app identity:** bundle id `com.zeus.zeus` (or similar), its own UserDefaults domain. zeus must not read/write Zeus's preferences. Shared on-disk things it may **read**: agent transcripts (`~/.claude/projects`, `~/.codex/sessions`) for usage/history — both read-only scans.
...
| JSON | `serde` + `serde_json` | Dates are **milliseconds since 1970** (`JSONEncoder.zeus`); sorted keys not required for a client. |
...
| Packaging | `cargo-packager` (universal binary, .app assembly, entitlements) + existing Apple ID / notarytool creds from Zeus's release flow | Updater: a minimal self-updater (`crates/zeus-updater`) on the same Cloudflare-worker host under `/zeus/`; Sparkle's appcast was not reusable, and neither was its keypair model. |
...
zeus/
  ...
  PLAN.md               # this file
  crates/
    zeus-proto/         # wire types: ControlMessage, Methods params/results, SessionRecord &
                        # friends, Frames codec, GridUpdate RLE codec, Paths. Pure, no IO. Heavily unit-tested.
    zeus-client/        # tokio: DaemonClient (control), SessionAttachment (data channel),
                        # reconnect/backoff/heartbeat/event-resume. No gpui dependency.
    zeus-term/          # the grid renderer: GridBuffer, TerminalElement (gpui), input encoding,
                        # selection, local scrollback viewport, find highlighting.
    zeus-ui/            # design system (tokens from §8), StatusGlyph, BrandLogos (SVG paths),
                        # shared components: rows, palette chrome, hover cards.
    zeus-app/           # binary: window/chrome, sidebar, terminal pane, palettes, settings,
...
Dependency direction: `zeus-app → {zeus-ui, zeus-term, zeus-client} → zeus-proto`. `zeus-proto` and `zeus-client` compile fast and are testable headless against a real daemon.

---

## 6. Protocol port spec (what `zeus-proto` + `zeus-client` implement)
...
Dependencies are noted; tasks with the same phase and no dep arrow are parallel. Each task = one agent, one worktree, PR into `main` touching only `zeus/`. **Every task's acceptance includes: `cargo build && cargo test && cargo clippy -- -D warnings` clean, and no files outside `zeus/` modified.**

### Phase 0 — Foundation (serial, 1 task)
- **T0 Workspace bootstrap.** Create the workspace per §5, pin gpui git rev, `create-gpui-app`-style hello window opening with `Blurred` background + hidden titlebar, CI-less build script (`zeus/scripts/build.sh`). Commit a `rust-toolchain.toml` (1.95+). Acceptance: `cargo run -p zeus-app` shows a blurred empty window with repositioned traffic lights.

### Phase 1 — Protocol core (parallel after T0)
- **T1 zeus-proto: control types.** All `Methods.swift` params/results + `SessionRecord`/`SessionStatus`/`AgentKind`/`AttentionLevel`/`Project`/etc as serde types (ms-epoch dates, lenient unknown-variant handling), `ControlMessage` envelope. Acceptance: round-trip tests against JSON fixtures **captured from the live daemon** (`nc -U` the socket or replay `state.json`), including a real `session.list` response.
- **T2 zeus-proto: frames + grid codec.** Binary frame codec + `GridUpdate` RLE decode/encode + scrollback-cells block decode. Acceptance: byte-exact round-trip property tests; a fixture captured from a live attach decodes to sensible cells.
- **T3 zeus-client: DaemonClient.** tokio UnixStream, NDJSON framing, request correlation, backoff reconnect, heartbeat, `events.subscribe` seq-resume, typed method wrappers, connection-state stream. **Hard rule from §3: no shutdown methods, no daemon spawning logic beyond "launch installed zeusd if socket absent".** Acceptance: integration test against the real local daemon — hello, list sessions, subscribe, spawn+kill a `shell` session in a temp dir.
- **T4 zeus-client: SessionAttachment.** Attach handshake, frame read/write loop, ping/pong keepalive, chunk stream (grid/modes), input/resize/scroll senders, caller-driven re-attach. Acceptance: headless integration test — spawn shell session, attach, send `echo hi\n`, assert the decoded grid contains `hi`; resize and assert reflow.

### Phase 2 — Terminal renderer (after T2; T5 can start on fixtures before T4 lands)
- **T5 zeus-term: GridBuffer + TerminalElement.** Cell model, GridUpdate application, batched quad/text painting, shaped-line cache, cursor (block + blink-on-focus), theme (16-color ANSI + default fg/bg/cursor/selection from §8 catalog), cell-metric layout, font fallbacks, rendering-suspended mode. Acceptance: a demo binary replaying a captured frame dump renders correctly (screenshot-compared against the Swift app for the same dump); idle draws zero frames.
- **T6 zeus-term: input encoding.** Port `TerminalKeyEncoding.swift` (+ paste). Acceptance: unit tests enumerating the Swift file's mappings; interactive check — fish, vim arrows, ctrl-c, opt-arrow word jump work in a live session.
- **T7 zeus-term: scroll + scrollback + selection + find.** Wheel routing (modes-aware), local scrollback viewport with `read_scrollback_cells` fetch/cache, selection + ⌘C, find model (debounced 200 ms, cap 500, live+history, highlight spans) + scrolled pill. Depends on T4, T5. Acceptance: scroll through 1000 lines of history, select across the seam, find wraps and anchors like the Swift app.

### Phase 3 — App shell (after T1, T3; parallel tracks)
- **T8 zeus-ui: design system.** Tokens (§8) as Rust constants/helpers, squircle backgrounds, hover/selected fill components, shared floating-surface recipe, motion (shared wall-clock phase), brand marks: port the 4 SVG `d` strings + a tiny path parser (or pre-convert to gpui `Path` ops at build time), `StatusGlyph` with all states/animations, `AgentLogo`. Acceptance: a gallery demo view showing every token/glyph state side-by-side with screenshots of the Swift originals.
...
- **T16 Perf pass.** Instruments/Metal HUD profiling against the §7 budgets; shaped-line cache hit rates; frame-time traces under `yes`-storm ×3 sessions; memory audit; fix regressions. Acceptance: all budget numbers recorded in `zeus/PERF.md` and met.
- **T17 Fidelity pass.** Pixel-diff sweep across every surface vs the Swift app (use `ZEUS_PREVIEW_SNAPSHOT` on the Swift side for reference PNGs where applicable); tune vibrancy tint per appearance; animation phase/feel review. Acceptance: annotated before/after gallery; sign-off list of intentional deviations (expected: blur tint nuance).
- **T18 Coexistence soak.** Run Zeus + zeus simultaneously for a day of real work: no daemon restarts, no resize thrash when different sessions are focused, documented behavior when the same session is focused in both, event-resume across daemon restart. Acceptance: written soak report; any protocol-level fix filed.

### Phase 6 — Ship
- **T19 Packaging.** cargo-packager config: universal binary, icons (new zeus icon), Info.plist, entitlements, codesign + notarize with existing creds, DMG; `scripts/release.sh` parallel to the Swift one. Acceptance: notarized zeus.app launches clean on a second Mac.
- **T20 Updater + docs.** Updater **done**: `crates/zeus-updater` + `crates/zeus-app/src/updates.rs`, JSON feed at `/zeus/appcast.json` on the existing worker, update pill in the sidebar footer, Settings → General → Updates, `⌘K → Check for Updates…`, published by `zeus/scripts/release.sh`. Trust is Developer ID + notarization pinned to the running app rather than a Sparkle-style keypair — see [UPDATING.md](UPDATING.md). Remaining: `zeus/README.md` (install, coexistence rules, known limitations), and the acceptance run — an old signed build updating itself to a new one, which needs a real notarized release.
...
| Risk | Mitigation |
|---|---|
| GPUI API churn (pre-1.0, git pin) | Pin one rev in T0; single owner for upgrades; wrap gpui-isms behind `zeus-ui` helpers where cheap. |
...
| Swift app updates daemon while zeus is open | zeus tolerates disconnect (backoff + seq-resume + full-snapshot re-attach) — explicitly tested in T18. |
| Missing kitty-keyboard protocol (daemon side uses SwiftTerm) | Non-issue for zeus: key encoding targets the same daemon the Swift app talks to today. |
