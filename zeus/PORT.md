...
| Holder (session survival) | **done** | Server, manager, launcher, client — same sockets, NDJSON protocol, pid files, and OSC 777 exit marker as `ZeusHolderKit`. Interop-proven live: the Rust client/launcher drives the real Swift `zeusd-holder` binary (spec encode → Swift decode, stat/write/resize/signal/kill-tree, Swift-written log read back, Swift exit marker parsed) |
...
Remaining gaps in `zeusd-rs` (all answer clean errors; the Swift daemon
