...
env ZEUS_SIDEBAR_PREVIEW=1 ZEUS_SIDEBAR_SCENARIO=stress ./scripts/dev.sh
env ZEUS_SIDEBAR_PREVIEW=1 ZEUS_SIDEBAR_SCENARIO=empty ./scripts/dev.sh
env ZEUS_SIDEBAR_PREVIEW=1 ZEUS_SIDEBAR_SCENARIO=artifacts ./scripts/dev.sh
```
...
them; Zeus neither requires nor configures Tailscale for Remote Holder sessions.

## Agent preferences

The default agent is stored in
`~/Library/Application Support/zeus/prefs.json` as the agent manifest's stable
...
on the next save. If a saved agent is removed or is no longer available, Zeus
