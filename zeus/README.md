...
`zeus` is a client of the installed Zeus daemon. It never replaces or automatically restarts `zeusd`; the sole explicit exception is Settings → Remote, where changing iPhone companion access asks the daemon to reload `remote.json`. Until the protocol gains multi-desktop geometry arbitration, do not focus the same session in Zeus and `zeus` at different terminal sizes; both desktop clients would resize the same PTY. Input sent from both apps is interleaved.
