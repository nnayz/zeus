...
As of 2026-08-09, the bootstrapped Rust Remote PTY Holder is Zeus's only remote
...
been removed. Zeus never falls back to `tmux`.
...
- **Remote Helper** and **`zeus-remote`** refer to the same minimal executable;
...
Zeus desktop app
  ...
  -> per-session zeus-remote Holder
...
- gives Zeus direct ownership of the remote Agent PTY lifecycle;
- requires no remote `tmux`, `screen`, `zellij`, Node.js, Python, `socat`, `nc`,
  `curl`, `wget`, or preinstalled Zeus service;
...
Zeus already uses Holder processes to decouple PTY, Agent, GUI, and daemon
...
The desktop app and `zeus-client` request remote operations through the Engine.
They do not execute SSH directly.

### Remote Helper

`zeus-remote` is deliberately not a second Zeus Engine. It owns only facts that
cannot remain local:
...
zeus-remote list
zeus-remote kill
zeus-remote environment
zeus-remote directories
zeus-remote persistence
zeus-remote gc
```

Each session has exactly one independent Holder and one Unix socket. There is no
multi-session remote Zeus supervisor. A Holder may spawn one minimal liveness
...
- `zeus-engine`: local session authority, host orchestration, bootstrap, SSH,
  reconnect, and status reduction;
- `zeus-proto::remote_pty`: versioned Helper protocol and wire codec;
- `zeus-pty`: shared low-level PTY primitives;
- `zeus-terminal-state`: shared headless parser, grid, snapshot, and diff model;
- `zeus-remote`: minimal remote Helper executable;
- `zeus-client`: app-to-Engine client only;
- `zeus-term`: GPUI terminal rendering and input integration;
- `zeus-app`: desktop UI and user actions;
- `zeus-node`: optional enhanced node mode, never required by SSH bootstrap.

`zeus-remote` does not depend on GPUI, `zeus-app`, `zeus-client`, `zeus-node`, or
...
`zeus-ssh-askpass`; the Engine does not parse passwords or host-key answers from
...
Zeus follows the useful part of Zed's remote model: establish SSH, inspect the
...
at commit `dc2a339`, but Zeus keeps its own narrower Holder boundary and does
...
The packaged Helper catalog is authoritative. Zeus never downloads an
...
variables, including local-only `ZEUS_` and `SSH_` state, are removed. The
...
- `zeus-node` runs only when the user explicitly enables that separate mode.
...
Each host is therefore probed rather than assumed. Zeus launches a temporary
...
Zeus never installs a service, persistent user unit, or LaunchAgent; never calls
...
Zeus prefers lower latency, CPU usage, memory use, copies, wakeups, and idle
...
## Tailscale, iPhone Companion, and `zeus-node`

These features are separate from Remote Holder transport:

- Tailscale may provide network reachability, but Zeus does not configure it and
  remote sessions do not depend on it;
- `zeus-node` is an optional, explicitly configured enhanced mode and is not a
...
```bash
ZEUS_REMOTE_SSH_TARGET=user@disposable-host \
ZEUS_REMOTE_SOAK_SECONDS=180 \
scripts/remote-ssh-soak.sh
```

Optional variables are `ZEUS_REMOTE_HELPER_PATH`,
`ZEUS_REMOTE_SSH_EXECUTABLE`, and `ZEUS_REMOTE_CWD`. The test prints its unique
...
- deeper, explicitly configured `zeus-node` integration.
...
- build a complete remote Zeus daemon;
