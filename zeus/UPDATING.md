...
3. Click **Restart to update to 0.2.0** → zeus hands off to a helper and quits.
...
process group, and quits. The helper waits (up to 60 s) for zeus's pid to
disappear, renames the old bundle to `zeus.app.zeus-previous`, unpacks the
...
Staging lives in `~/Library/Caches/zeus/updates/<version>/`, with the helper's
...
1. Keep a copy of the previous `zeus.app` (or install the previous DMG).
...
- **"Updates off for this build."** The running app is not in a `.app`, or is
  ...
  without `ZEUS_SIGN_IDENTITY`. Settings → General shows the exact reason.
...
- **"zeus can't write to its own folder."** The app is in `/Applications` on a
...
- **An install went wrong.** `~/Library/Caches/zeus/updates/<version>/install.log`
  holds the helper's output, and `zeus.app.zeus-previous` next to the app is
