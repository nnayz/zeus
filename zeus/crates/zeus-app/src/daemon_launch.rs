//! Launch the Swift daemon (`zeusd`) that ships bundled inside `zeus.app`.
//!
//! zeus talks to the existing Zeus daemon over a Unix socket
//! (`~/Library/Application Support/Zeus/daemon.sock`). Historically the
//! legacy `Zeus.app` was responsible for spawning that daemon; now that zeus
...
/// Environment override pointing directly at a `zeusd` executable, mirroring
/// the Swift `ZEUS_HOLDER_PATH` seam used by `HolderLauncher`.
const ENV_DAEMON_PATH: &str = "ZEUSD_PATH";

const BOOT_LOG_FILE_NAME: &str = "zeusd.boot.log";

/// Ensure a daemon is reachable at `socket_path`, spawning the bundled
/// `zeusd` detached if the socket is dead.
///
...
pub fn ensure_daemon_running(socket_path: &Path) {
    ...
    match resolve_daemon_path() {
        ...
        None => eprintln!(
            "zeus: no bundled zeusd found next to the executable; \
...
/// Resolve the `zeusd` executable to launch, using the live process layout.
...
///   2. Bundled: `Contents/MacOS/zeus` → `../Resources/bin/zeusd`.
///   3. Next to the executable (loose dev copy).
///   4. Swift SPM build outputs under the working dir: `.build/{release,debug}/zeusd`.
fn resolve_daemon_path_from(
    ...
    {
        // Contents/MacOS/zeus → Contents/Resources/bin/zeusd
        if let Some(contents) = macos_dir.parent() {
            candidates.push(contents.join("Resources/bin/zeusd"));
        }
        // Loose copy sitting right next to the executable.
        candidates.push(macos_dir.join("zeusd"));
    }

    if let Some(cwd) = current_dir {
        candidates.push(cwd.join(".build/release/zeusd"));
        candidates.push(cwd.join(".build/debug/zeusd"));
    }

    candidates.into_iter().find(|path| is_executable(path))
}

/// Spawn `zeusd` in its own process group so it outlives zeus, with
/// stdout/stderr appended to `zeusd.boot.log` (the same boot log the legacy
...
/// `~/Library/Application Support/Zeus/zeusd.boot.log`, creating the
...
mod tests {
    ...
    fn resolves_bundled_daemon_from_contents_macos() {
        ...
        let daemon = root.join("Contents/Resources/bin/zeusd");
    ...
    fn env_override_wins_when_executable() {
        let tmp = tempfile::tempdir().unwrap();
        let override_path = tmp.path().join("custom/zeusd");
    ...
    fn ignores_non_executable_override_and_falls_back_next_to_exe() {
        ...
        touch_executable(&exe);
        let sibling = tmp.path().join("bin/zeusd");
