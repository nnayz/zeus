//! Launch the authoritative Rust Engine (`zeusd-rs`) bundled in `zeus.app`.
//!
//! zeus talks to the Engine over its owner-only Unix socket. The remote PTY
...
pub fn ensure_daemon_running(socket_path: &Path) {
    ...
    match probe_daemon(socket_path) {
        Ok(hello) if hello.engine_kind.as_deref() == Some(RUST_ENGINE_KIND) => {
            ...
            let expected_hash = match executable_sha256(daemon) {
                ...
                Err(error) => {
                    eprintln!(
                        "zeus: cannot verify bundled daemon {}: {error}; keeping the live Engine",
                        daemon.display()
                    );
                    return;
                }
            ...
            eprintln!(
                "zeus: refreshing Rust Engine {:?} from bundled executable {}",
                hello.build,
                daemon.display()
            );
            if let Err(error) = stop_daemon_for_upgrade(socket_path) {
                eprintln!(
                    "zeus: could not stop the outdated Rust Engine at {}: {error}",
                    socket_path.display()
                );
                return;
            }
        ...
        Ok(hello) => {
            ...
            // are re-adopted.
            eprintln!(
                "zeus: replacing non-Rust daemon build {:?} at {} with the bundled Rust Engine",
                hello.build,
                socket_path.display()
            );
            if daemon.is_none() {
                eprintln!(
                    "zeus: no bundled Engine to replace it with; leaving {} alone",
                    socket_path.display()
                );
                return;
            }
            if let Err(error) = stop_daemon_for_upgrade(socket_path) {
                eprintln!(
                    "zeus: could not stop the previous daemon at {}: {error}",
                    socket_path.display()
                );
                return;
            }
        ...
        Err(error) if probe_daemon(socket_path).is_err() => {
            eprintln!(
                "zeus: replacing the Engine at {} — it could not answer the identity probe ({error})",
                socket_path.display()
            );
            if daemon.is_none() {
                eprintln!("zeus: no bundled Engine to replace it with; leaving it alone");
                return;
            }
            if let Err(error) = stop_daemon_for_upgrade(socket_path) {
                eprintln!(
                    "zeus: could not stop the unidentified Engine at {}: {error}",
                    socket_path.display()
                );
                return;
            }
        ...
        Err(error) => {
            eprintln!(
                "zeus: the Engine at {} answered a retried identity probe; leaving it alone ({error})",
                socket_path.display()
            );
            return;
        }
    ...
    match daemon {
        Some(daemon) => match spawn_detached(&daemon) {
            Ok(()) => eprintln!("zeus: launched bundled daemon at {}", daemon.display()),
            Err(err) => {
                eprintln!(
                    "zeus: failed to launch bundled daemon {}: {err}",
                    daemon.display()
                );
            }
        },
        None => eprintln!(
            "zeus: no bundled zeusd-rs found next to the executable; \
...
fn probe_daemon(socket_path: &Path) -> io::Result<HelloResult> {
    let params =
        serde_json::to_value(HelloParams::new("zeus-launch-probe")).map_err(io::Error::other)?;
...
fn control_request(
    ...
    reader
        .by_ref()
        .take(zeus_proto::control::MAX_CONTROL_LINE_BYTES as u64 + 1)
        .read_until(b'\n', &mut response)?;
    if response.len() > zeus_proto::control::MAX_CONTROL_LINE_BYTES {
...
///   1. `ZEUSD_PATH` override (dev/tests).
///   2. Bundled: `Contents/MacOS/zeus` → `../Resources/bin/zeusd-rs`.
///   3. Next to the executable (loose dev copy).
///   4. Cargo build outputs under the working dir: `target/{release,debug}/zeusd-rs`.
fn resolve_daemon_path_from(
    ...
    {
        // Contents/MacOS/zeus → Contents/Resources/bin/zeusd-rs
        if let Some(contents) = macos_dir.parent() {
            candidates.push(contents.join("Resources/bin/zeusd-rs"));
        }
        // Loose copy sitting right next to the executable.
        candidates.push(macos_dir.join("zeusd-rs"));
    }

    if let Some(cwd) = current_dir {
        candidates.push(cwd.join("target/release/zeusd-rs"));
        candidates.push(cwd.join("target/debug/zeusd-rs"));
    }

    candidates.into_iter().find(|path| is_executable(path))
}

/// Spawn the Engine in its own process group so it outlives zeus, with
/// stdout/stderr appended to `zeusd-rs.boot.log`. We never wait on the
...
fn spawn_detached(daemon: &Path) -> io::Result<()> {
    ...
    // from zeus's signal/terminal group so quitting zeus never SIGHUPs the
...
/// `~/Library/Application Support/Zeus/zeusd-rs.boot.log`, creating the
/// support directory if needed. Returns `None` when `HOME` is unset.
fn boot_log_path() -> Option<PathBuf> {
    let home = std::env::var_os("HOME")?;
    let support = ZeusPaths::app_support(PathBuf::from(home));
...
mod tests {
    ...
    fn resolves_bundled_daemon_from_contents_macos() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path();
        let exe = root.join("Contents/MacOS/zeus");
        touch_executable(&exe);
        let daemon = root.join("Contents/Resources/bin/zeusd-rs");
    ...
    fn env_override_wins_when_executable() {
        let tmp = tempfile::tempdir().unwrap();
        let override_path = tmp.path().join("custom/zeusd-rs");
    ...
    fn ignores_non_executable_override_and_falls_back_next_to_exe() {
        ...
        let exe = tmp.path().join("bin/zeus");
        touch_executable(&exe);
        let sibling = tmp.path().join("bin/zeusd-rs");
    ...
    fn returns_none_when_nothing_resolves() {
        let tmp = tempfile::tempdir().unwrap();
        let exe = tmp.path().join("Contents/MacOS/zeus");
        touch_executable(&exe);
