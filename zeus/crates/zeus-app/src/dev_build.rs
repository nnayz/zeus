const DEV_BUNDLE_PREFIX: &str = "com.zeus.zeus.dev.";
...
impl DevBuildIdentity {
    pub(crate) fn from_process_environment(bundle_id: Option<&str>) -> Option<Self> {
        Self::from_parts(
            std::env::var("ZEUS_DEV").ok().as_deref(),
            bundle_id,
            std::env::var("ZEUS_DEV_BUILD").ok().as_deref(),
        )
    }
...
mod tests {
    ...
    fn explicit_dev_bundle_enables_identity() {
        ...
        assert_eq!(identity.bundle_id(), "com.zeus.zeus.dev.abc1234");
        assert_eq!(identity.window_title(), "zeus dev — worktree@abc1234");
    ...
    fn opt_in_and_build_label_are_both_required() {
        let bundle = Some("com.zeus.zeus.dev.abc1234");
