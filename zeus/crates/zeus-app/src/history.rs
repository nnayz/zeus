...
//!
//! This is a Rust port of `ZeusDaemonKit/HistoryScanner.swift`. Keeping the
//! scan client-side lets zeus show durable history without requiring a newer
...
mod tests {
    ...
    fn fuzzy_filter_is_case_insensitive_subsequence() {
        ...
        assert!(matches_query(&entry, "zeus"));
    ...
    #[ignore = "set ZEUS_LIVE_HISTORY_TEST=1 to resume a real Claude conversation"]
    async fn live_historical_claude_resume_smoke() {
        assert_eq!(std::env::var("ZEUS_LIVE_HISTORY_TEST").as_deref(), Ok("1"));
        let client = DaemonClient::new();
        client.connect();
        client
            .wait_until_connected(Duration::from_secs(8))
            .await
            .expect("live Zeus daemon is not reachable");
        ...
        let tracked = current
            ...
            .collect();
        let requested_id = std::env::var("ZEUS_HISTORY_ID").ok();
        ...
        let session_id = if let Ok(existing) = std::env::var("ZEUS_EXISTING_SESSION_ID") {
            zeus_proto::SessionId::new(existing)
        } else {
            client
                .spawn(params)
                .await
