...
use zeus_proto::{AgentKind, AttentionLevel, RiskHint, SessionId, SessionRecord, SessionStatus};
...
impl NativeMenuBar {
    ...
    pub fn new(mtm: MainThreadMarker, store: Arc<RwLock<SessionStore>>) -> Option<Self> {
        ...
        unsafe {
            ...
            button.setAction(Some(sel!(toggleZeusMenu:)));
        }
        ...
        let button = unsafe {
            NSButton::buttonWithTitle_target_action(
                &NSString::from_str(&format!(
                    "{remaining} more session{} in zeus",
...
define_class!(
    ...
    impl MenuBarTarget {
        ...
        #[unsafe(method(openZeus:))]
        fn open_zeus(&self, _sender: Option<&AnyObject>) {
        ...
        #[unsafe(method(quitZeus:))]
        fn quit_zeus(&self, _sender: Option<&AnyObject>) {
