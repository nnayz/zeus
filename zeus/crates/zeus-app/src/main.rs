...
fn install_app_menus(cx: &mut App) {
    ...
    cx.set_menus([
        Menu::new("zeus").items([
            ...
            MenuItem::separator(),
            MenuItem::action("Hide zeus", HideApp),
            MenuItem::separator(),
...
fn main() {
    #[cfg(target_os = "macos")]
    if std::env::var_os("ZEUS_PROBE_SYMBOLS").is_some() {
        macos::sf_symbols::probe();
        return;
    }

    let preview_value = std::env::var("ZEUS_SIDEBAR_PREVIEW").ok();
    let preview = preview_value.as_deref().is_some_and(|value| value != "0");
    let scenario_value = std::env::var("ZEUS_SIDEBAR_SCENARIO")
        .ok()
    ...
    let tokio = Arc::new(
        RuntimeBuilder::new_multi_thread()
            .worker_threads(2)
            .thread_name("zeus-async")
            .enable_all()
            .build()
            .expect("failed to start zeus async runtime"),
    );

    // zeus is self-contained: if no daemon socket is live, launch the daemon
    ...
    if !preview {
        ...
        let socket_path = zeus_proto::paths::ZeusPaths::socket(home);
    ...
    let app = application().with_assets(zeus_ui::IconAssets);
    ...
    app.run(move |cx: &mut App| {
        ...
        #[cfg(target_os = "macos")]
        zeus_ui::set_mark_rasterizer(macos::brand_raster::raster_mark);
        bind_terminal_keys(cx);
        install_app_menus(cx);
        let quit_store = Arc::clone(&services.store);
        let release_owned_daemon =
            !preview && std::env::var_os(zeus_proto::paths::ENV_SOCKET).is_none();
        cx.on_app_quit(move |_| {
            ...
            async move {
                if let Err(error) = quit_store
                    ...
                    .persist_preferences()
                {
                    eprintln!("zeus: could not flush preferences while quitting: {error}");
                }
                if release_owned_daemon
                    && let Err(error) = quit_store.client().shutdown_daemon_if_idle().await
                {
                    eprintln!("zeus: could not release the idle Engine while quitting: {error}");
                }
...
fn open_main_window(
    ...
    let perf_large_window = std::env::var_os("ZEUS_PERF_LARGE_WINDOW").is_some();
