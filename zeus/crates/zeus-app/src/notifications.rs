...
pub fn prefs_sync_transition(
    host_name: &str,
    result: Result<&zeus_proto::HostSyncPrefsResult, &str>,
) -> StatusTransition {
    match result {
        Ok(report) => {
...
fn blocker_identity(detail: &zeus_proto::NeedsInputDetail) -> String {
...
mod tests {
    ...
    fn prefs_sync_and_migration_banners_summarize_outcomes() {
        let report = zeus_proto::HostSyncPrefsResult {
            tools: vec![
                zeus_proto::PrefsSyncToolReport {
                    tool: "claude".into(),
                    ok: true,
