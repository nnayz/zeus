...
impl CompanionAccess {
    pub fn load(home: &Path) -> Self {
        let config = RemoteConfig::load(zeus_proto::paths::ZeusPaths::remote_config_file(home));
        Self {
            config,
...
pub fn enable(home: &Path) -> Result<CompanionAccess, String> {
    ...
    let path = zeus_proto::paths::ZeusPaths::remote_config_file(home);
...
mod tests {
    ...
    fn pairing_link_prefers_magic_dns_and_matches_the_ios_parser() {
        ...
        assert_eq!(
            ...
            Some("zeus://studio.example.ts.net:48620?token=secret-token")
        );
