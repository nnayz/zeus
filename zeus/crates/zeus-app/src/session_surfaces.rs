...
use zeus_term::element::{SharedGridBuffer, TerminalElement};
use zeus_ui::{
...
pub struct SessionSurfaces {
    ...
    status_glyphs: HashMap<(SessionId, u16, zeus_ui::AgentKind), Entity<StatusGlyph>>,
...
fn state_badge_color(session: &SessionRecord, colors: SemanticColors) -> gpui::Rgba {
    if session
        .hibernation
        .as_ref()
        .is_some_and(|info| info.reason == zeus_proto::HibernationReason::MemoryPressure)
        || session
            .memory_bytes
