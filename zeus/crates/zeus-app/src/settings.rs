...
//!
//! General, Terminal, and Resources mutate zeus's preferences. Remote manages
//! the shared execution-host catalog used by the SSH Remote Holder transport.

use zeus_proto::{AgentKind, AgentReadinessResult, HostEntry, HostNodeConfig};
...
mod tests {
    ...
    fn terminal_theme_falls_back_and_wraps() {
        assert_eq!(theme("missing").id, TermTheme::ZEUS_DARK.id);
        let last = TermTheme::CATALOG.last().unwrap();
        assert_eq!(next_theme(last.id).id, TermTheme::ZEUS_DARK.id);
    }
    ...
    fn host_drafts_reject_missing_or_malformed_destinations() {
        ...
        draft.node_token_file = "~/.config/zeus/builder.token".into();
