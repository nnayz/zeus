...
impl Sidebar {
        ...
        let running = pending
            ...
            .filter(|id| {
                store.sessions().get(*id).is_some_and(|session| {
                    !matches!(session.status, zeus_proto::SessionStatus::Exited(_))
                })
            })
            .count();
    ...
    pub fn toggle(&mut self, cx: &mut Context<Self>) {
        ...
        {
            eprintln!("zeus: could not remember sidebar visibility: {error}");
        }
    ...
    fn session_row(
        ...
        let ended = matches!(session.status, zeus_proto::SessionStatus::Exited(_)) && !archived;
    ...
    ///
    /// This is the whole of zeus's update UI in the main window, and it stays
    ...
    fn session_actions_popover(
        ...
        if bulk.len() > 1 {
            ...
            let running = !matches!(session.status, zeus_proto::SessionStatus::Exited(_));
            if !running && session.resumability == zeus_proto::Resumability::Resumable {
                content = content.child(menu_row(
                    "Resume",
                    colors,
                    cx.listener({
            ...
            content = content
                .child(menu_row(
                    ...
                    if session.resumability == zeus_proto::Resumability::NotResumable {
...
fn display_title(session: &SessionRecord) -> String {
    if session.title_source == zeus_proto::TitleSource::Placeholder {
        if matches!(
            session.status,
            zeus_proto::SessionStatus::Starting
                | zeus_proto::SessionStatus::Working
                | zeus_proto::SessionStatus::NeedsInput(_)
        ) {
            "Untitled".into()
        } else {
            "Ended".into()
    ...
    match session.attention() {
        ProtoAttentionLevel::NeedsInput => StatusState::NeedsInput {
            destructive: session
                .needs_input
                .as_ref()
                .is_some_and(|detail| detail.risk_hint == zeus_proto::RiskHint::Destructive),
        },
...
fn agent_picker_options(catalog: &zeus_proto::AgentReadinessResult) -> Vec<AgentPickerOption> {
...
mod tests {
    ...
    fn agent_picker_keeps_manifest_only_and_unavailable_guidance_rows() {
        let catalog = zeus_proto::AgentReadinessResult {
            agents: vec![
                zeus_proto::AgentReadinessItem {
                    kind: ProtoAgentKind::new("amp"),
                    binary: "amp".into(),
                    path: Some("/bin/amp".into()),
                    descriptor: Some(zeus_proto::AgentDescriptor {
                        id: "amp".into(),
                ...
                zeus_proto::AgentReadinessItem {
                    ...
                    descriptor: Some(zeus_proto::AgentDescriptor {
                        ...
                        setup: Some(zeus_proto::AgentSetup {
    ...
    fn remote_directory_navigation_keeps_the_explicit_child_path() {
        assert_eq!(
            remote_picker_target(Some("/Users/remote/code/zeus"), Some("~")),
            "/Users/remote/code/zeus"
        );
    }
    ...
    /// A sidebar full of working Agents is zeus's normal resting state, so a
    ...
    /// last frame. `zeus-ui`'s `status_marks_never_sample_a_clock_while_rendering`
    ...
    fn project_plus_opens_the_agent_kind_menu_in_that_project(cx: &mut TestAppContext) {
        ...
        let project = cx
            .debug_bounds("PROJECT_preview-zeus")
            .expect("project row");
        cx.simulate_mouse_move(project.center(), None, Modifiers::default());
        let plus = cx
            .debug_bounds("PROJECT_ADD_preview-zeus")
        ...
        assert_eq!(
            ...
            Some(Popover::NewAgent {
                directory: Some("/Users/preview/Projects/zeus".to_owned()),
                host: None,
            })
        );
        ...
        let (view, cx) = cx.add_window_view(|_, cx| {
            let sidebar = cx.new(|cx| {
                ...
                sidebar
                    ...
                    .write()
                    .expect("session store lock poisoned")
                    .set_hosts(vec![zeus_proto::HostEntry {
                        id: "forge".into(),
