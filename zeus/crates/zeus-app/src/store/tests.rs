...
fn attention_rollup_and_needs_input_sort_use_proto_derivation() {
    ...
    older_input.status = SessionStatus::NeedsInput(zeus_proto::NeedsInputKind::Question);
    older_input.updated_at = DateMillis(100.0);
    let mut newer_input = session("newer-input", "p", 3.0);
    newer_input.status = SessionStatus::NeedsInput(zeus_proto::NeedsInputKind::Permission);
...
fn hidden_needs_input_update_emits_chime_and_notification_effect() {
    ...
    hidden.status = SessionStatus::NeedsInput(zeus_proto::NeedsInputKind::Permission);
...
fn synthetic_events_upsert_project_and_remove_with_neighbor_focus() {
    ...
    store.handle_event(EventEnvelope {
        name: zeus_proto::EventName::SESSION_UPDATED.to_owned(),
        seq: 1,
    ...
    store.handle_event(EventEnvelope {
        name: zeus_proto::EventName::PROJECT_UPDATED.to_owned(),
        seq: 2,
    ...
    store.handle_event(EventEnvelope {
        name: zeus_proto::EventName::SESSION_REMOVED.to_owned(),
        seq: 3,
...
fn compact_resource_events_patch_only_resource_fields() {
    ...
    assert!(store.handle_event(EventEnvelope {
        name: zeus_proto::EventName::SESSION_RESOURCES.to_owned(),
        seq: 1,
