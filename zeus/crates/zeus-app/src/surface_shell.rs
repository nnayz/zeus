...
const SETTINGS_COLORS: SemanticColors = SemanticColors::sidebar(zeus_ui::Appearance::Dark);
...
enum HostInitialization {
    ...
    Ready {
        ...
        kind: HostPreparationKind,
        operation: u64,
        result: zeus_proto::HostInitializeResult,
    },
    Failed {
        id: String,
        name: String,
...
impl UtilitySurfaces {
    pub fn new(
        ...
        let settings_preview = std::env::var("ZEUS_SETTINGS_PREVIEW")
            .ok()
    ...
    fn persist_prefs(&mut self) {
            ...
            .update_preferences(|shared| *shared = prefs)
        {
            ...
            self.activity = "Settings saved for zeus".to_owned();
        }
    }
    ...
    /// the button here (and the sidebar pill), because zeus holds live agent
            ...
            div()
                ...
                .child(
                    settings_note(
                        ...
                        "Zeus uses your SSH configuration without changing the host. Private-network and Tailscale names work transparently when OpenSSH can resolve them.",
                        SETTINGS_COLORS.secondary,
        ...
        let HostInitializationCardModel {
            ...
            HostInitialization::Running { id, name, kind, .. } => {
                let (title, detail) = match kind {
                    HostPreparationKind::Initialize => (
                        ...
                        "Connecting with SSH, verifying zeus-remote, loading the login environment, and testing session persistence."
                            .to_owned(),
                    ),
                    HostPreparationKind::Reinstall => (
                        format!("Reinstalling {name}"),
                        "Uploading and verifying the packaged zeus-remote build, then refreshing the remote environment checks. Running sessions are not interrupted."
                            .to_owned(),
                    ),
                };
            ...
            HostInitialization::Ready {
                ...
                let persistence = match result.persistence {
                    zeus_proto::remote_pty::PersistenceCapability::NativeDetach => "native detach",
                    zeus_proto::remote_pty::PersistenceCapability::UserSupervisor => {
                        "user supervisor"
                    }
...
fn ui_default_agent(agent: DefaultAgent) -> zeus_ui::AgentKind {
    match agent {
        DefaultAgent::ClaudeCode => zeus_ui::AgentKind::ClaudeCode,
        DefaultAgent::Codex => zeus_ui::AgentKind::Codex,
        DefaultAgent::Cursor => zeus_ui::AgentKind::Cursor,
        DefaultAgent::Gemini => zeus_ui::AgentKind::Gemini,
    }
}
...
mod tests {
    ...
    fn completed_reinstall_expires_without_clearing_a_newer_operation() {
        let result = zeus_proto::HostInitializeResult {
            helper_build_id: "test-build".into(),
            protocol: zeus_proto::remote_pty::ProtocolVersion::CURRENT,
            persistence: zeus_proto::remote_pty::PersistenceCapability::NativeDetach,
