...
use zeus_proto::{AttentionLevel, SessionId, SessionRecord, SessionStatus, TitleSource};
...
mod tests {
    ...
    fn overview_lanes_are_partitioned_by_lifecycle_then_attention() {
        ...
        let waiting = session(
            "waiting",
            SessionStatus::NeedsInput(zeus_proto::NeedsInputKind::Question),
        );
        let ended = session(
            "ended",
        ...
        asleep.hibernation = Some(zeus_proto::HibernationInfo {
            reason: zeus_proto::HibernationReason::Idle,
    ...
    fn board_arrows_preserve_row_across_nonempty_lanes_and_list_is_linear() {
        let sessions = vec![
            ...
            session(
                "wait-1",
                SessionStatus::NeedsInput(zeus_proto::NeedsInputKind::Permission),
            ),
            session(
                "wait-2",
                SessionStatus::NeedsInput(zeus_proto::NeedsInputKind::Question),
