...
pub async fn merge_fleet_usage(mut snapshot: UsageSnapshot, home: &Path) -> UsageSnapshot {
    let hosts = HostsConfig::load(ZeusPaths::hosts_config_file(home));
...
fn merge_window(
    ...
    today: &zeus_proto::UsageQueryResult,
    month: &zeus_proto::UsageQueryResult,
...
fn convert(totals: &zeus_proto::UsageTotals, authoritative: bool) -> UsageTotals {
    UsageTotals {
