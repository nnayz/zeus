...
//!
//! This mirrors `Sources/ZeusClient/SessionAttachment.swift`: a fresh Unix
...
mod tests {
    ...
    impl TestControl {
        async fn connect(socket: &Path) -> Result<Self, Box<dyn Error>> {
            ...
            let _: HelloResult = control
                .request(Method::HELLO, &HelloParams::new("zeus-t4-integration"))
                .await?;
            Ok(control)
        }
    ...
    fn daemon_socket() -> Option<PathBuf> {
        if let Some(path) = std::env::var_os(ZeusEnv::SOCKET) {
            return Some(PathBuf::from(path));
        }
        std::env::var_os("HOME").map(ZeusPaths::socket)
    }
    ...
    fn scratch_title() -> String {
        ...
        format!("zeus-t4-test-{}-{nanos}", std::process::id())
    }
    ...
    async fn live_shell_attachment_renders_input_and_resize() -> Result<(), Box<dyn Error>> {
        ...
        let scratch = tempfile::Builder::new().prefix("zeus-t4-").tempdir()?;
        ...
        let result: Result<(), Box<dyn Error>> = async {
            ...
            attachment.send_input(b"echo zeus_test_marker\n".to_vec())?;
            timeout(Duration::from_secs(15), async {
                while let Some(chunk) = attachment.chunks.recv().await {
                    if let TerminalChunk::Grid(update) = chunk {
                        update.apply(&mut cells);
                        if composed_text(&cells).contains("zeus_test_marker") {
                            return Ok::<(), Box<dyn Error>>(());
                        }
                    }
                }
