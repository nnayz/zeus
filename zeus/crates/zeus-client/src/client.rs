...
impl ClientCore {
    /// The only event kinds zeus asks the daemon for.
    ///
    ...
    /// derived from a record change zeus already receives in full via
    ...
    /// gives that back.
    ///
    /// IF YOU ADD AN EVENT KIND THAT ZEUS NEEDS, ADD IT HERE TOO. Server-side
    ...
    async fn hello(&self, timeout: Option<Duration>) -> Result<HelloResult, ClientError> {
        let params = HelloParams {
            proto: zeus_proto::control::WIRE_VERSION,
...
/// A reconnecting, request-correlated control client for the existing Zeus daemon.
...
impl DaemonClient {
    /// Uses `~/Library/Application Support/Zeus/daemon.sock`.
    pub fn new() -> Self {
        let home = std::env::var_os("HOME")
            .map(PathBuf::from)
            .unwrap_or_else(|| PathBuf::from("/nonexistent"));
        Self::with_socket_path(ZeusPaths::socket(home))
    }
    ...
    /// single global daemon operation exposed by zeus: Settings writes
...
mod tests {
    ...
    async fn live_daemon_control_round_trip() -> Result<(), Box<dyn Error>> {
        if std::env::var_os("ZEUS_RUN_MUTATING_DAEMON_TESTS").is_none() {
            eprintln!(
                "skipping mutating live daemon test; set ZEUS_RUN_MUTATING_DAEMON_TESTS=1 to opt in"
            );
            return Ok(());
        }
        ...
        let socket = ZeusPaths::socket(home);
        if !socket.exists() {
            eprintln!(
        ...
        let test_result = async {
            ...
            if hello.proto != zeus_proto::control::WIRE_VERSION {
            ...
            let spawned_id = client
                .spawn(SessionSpawnParams {
                    ...
                    title: Some("zeus-client integration scratch".to_owned()),
                    initial_prompt: None,
                    parent: None,
