...
#[cfg(unix)]
use zeus_engine::holder::{HolderManagerServer, HolderServer};

#[cfg(not(unix))]
fn main() {
    eprintln!("zeus-holder requires a unix platform");
    std::process::exit(64);
}
...
fn main() {
    ...
    let result = if let Some(directory) = value_after(&arguments, "--manager") {
        ...
        let idle = std::env::var("ZEUS_HOLDER_IDLE_SECONDS")
            .ok()
        ...
        match std::fs::read(&spec_path) {
            Ok(data) => {
                ...
                match serde_json::from_slice(&data) {
                    ...
                    Err(error) => {
                        eprintln!("zeus-holder: spec did not parse: {error}");
                        std::process::exit(1);
                    }
                }
            }
            Err(error) => {
                eprintln!("zeus-holder: read {spec_path}: {error}");
                std::process::exit(1);
            }
        }
    } else {
        eprintln!("usage: zeus-holder --manager <directory> | --spec <path>");
        std::process::exit(64);
    };

    if let Err(error) = result {
        eprintln!("zeus-holder: {error}");
        std::process::exit(1);
    }
}
