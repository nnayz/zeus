...
//! talk to. The wire format is not ours to choose: `zeus-client` already speaks
...
/// become hook-driven and get the zeus MCP tools.
#[derive(Clone, Debug)]
...
impl ControlServer {
    ...
    pub fn spawn_remote_restore(self: &Arc<Self>) {
        ...
        if let Err(error) = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if !adopted.is_empty() {
                    eprintln!(
                        "zeus-engine: adopted {} remote Holder session(s): {adopted:?}",
                        adopted.len()
                    );
                }
            })
        {
            eprintln!("zeus-engine: could not start remote session restore: {error}");
        }
    }
    ...
    fn retire_legacy_remote_sessions(&self) {
        let plan = crate::legacy_remote::Plan {
            ...
            hosts: &zeus_proto::HostsConfig::load(self.hosts_file()),
    ...
    fn restore_remote_bindings(
        ...
        let hosts = zeus_proto::HostsConfig::load(self.hosts_file());
        ...
        for binding in bindings {
            ...
            let selector = zeus_proto::remote_pty::SessionSelector {
            ...
            if matches!(record.status, zeus_proto::SessionStatus::Exited(_))
                || matches!(
                    inspection.process_state,
                    zeus_proto::remote_pty::RemoteProcessState::Exited { .. }
                )
            {
            ...
            if !matches!(
                inspection.process_state,
                zeus_proto::remote_pty::RemoteProcessState::Running { .. }
            ) {
                continue;
            }
    ...
    fn events_subscribe(
        ...
        let p: zeus_proto::EventsSubscribeParams = decode(params).unwrap_or_default();
    ...
    fn events_wait(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::EventsWaitParams = decode(params)?;
        if p.until.is_empty() {
        ...
        let stream = self.events.subscribe(
            ...
            crate::events::Filter::new(
                ...
                Some(vec![zeus_proto::EventName::SESSION_UPDATED.to_string()]),
            ),
        );

        let current = |registry: &Registry| -> Option<zeus_proto::SessionRecord> {
            registry
                .records()
                .into_iter()
                .find(|record| record.id.0 == p.session_id.0)
        };
        let matches = |record: &zeus_proto::SessionRecord| {
            p.until
                .iter()
        ...
        loop {
            if matches(&latest) {
                return encode(&zeus_proto::EventsWaitResult {
                    session: latest,
                    timed_out: false,
                });
            }
    ...
    fn hello(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        ...
        Ok(json!({
            "proto": WIRE_VERSION,
            "build": BUILD,
            "engineKind": zeus_proto::RUST_ENGINE_KIND,
    ...
    fn session_spawn(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        ...
        let argv: Vec<String> = raw
            ...
            .unwrap_or_default();
        let p: zeus_proto::SessionSpawnParams = decode(Some(raw))?;
        if p.host.is_some() {
        ...
        let argv = if argv.is_empty() {
            match p.kind.command() {
                ...
                _ if kind == zeus_proto::AgentKind::SHELL_ID => {
                    let shell = std::env::var("SHELL").unwrap_or_else(|_| "/bin/zsh".into());
                    vec![shell, "-l".into()]
                }
                _ => Vec::new(),
        ...
        if let Some(title) = &p.title {
            ...
            record.title_source = zeus_proto::TitleSource::ZeusAssigned;
        }
        ...
        if kind == zeus_proto::AgentKind::CLAUDE_CODE_ID || prompt.is_some() {
            ...
            std::thread::spawn(move || {
                prepare_agent_input(
                    &registry,
                    &session_id,
                    kind == zeus_proto::AgentKind::CLAUDE_CODE_ID,
                    prompt.as_deref(),
                );
            });
        }
    ...
    fn session_spawn_remote(
        &self,
        p: zeus_proto::SessionSpawnParams,
        ...
        let captured = manager
            .capture_environment(
                &helper,
                &zeus_proto::remote_pty::EnvironmentCaptureRequest {
        ...
        } else if kind == zeus_proto::AgentKind::SHELL_ID {
        ...
        let launch = zeus_proto::remote_pty::LaunchRequest {
            session_id: id.clone(),
            session_token: token,
            argv: pty.argv.clone(),
            ...
            environment: pty
                ...
                .iter()
                .map(
                    |(name, value)| zeus_proto::remote_pty::EnvironmentVariable {
                        name: name.clone(),
                        value: value.clone(),
                    },
                )
    ...
    fn worktree_overview(&self) -> Result<JsonValue, ControlError> {
        ...
        let mut session_by_path: std::collections::HashMap<String, &zeus_proto::SessionRecord> =
            std::collections::HashMap::new();
        let running = |record: &zeus_proto::SessionRecord| {
            !matches!(
                record.status,
                zeus_proto::SessionStatus::Exited(_) | zeus_proto::SessionStatus::Unknown
            )
        };
        ...
        for root in roots {
            ...
            for worktree in worktrees {
                ...
                entries.push(zeus_proto::WorktreeOverviewEntry {
        ...
        encode(&zeus_proto::WorktreeOverviewResult { entries })
    }
    ...
    fn session_migrate(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::SessionMigrateParams = decode(params)?;
        ...
        if record.kind.id() != zeus_proto::AgentKind::CLAUDE_CODE_ID {
            return Err(ControlError::bad_request(
        ...
        // host + cwd.
        {
            ...
            registry.update_record(&id, |record| {
                ...
                record.status = zeus_proto::SessionStatus::Exited(zeus_proto::ExitInfo {
                ...
                record.resumability = zeus_proto::Resumability::Resumable;
            });
            let _ = registry.persist();
            self.publish_updated(&registry, &id);
        }
        ...
        let session: zeus_proto::SessionRecord = serde_json::from_value(revived)
            .map_err(|error| ControlError::internal(error.to_string()))?;
        encode(&zeus_proto::SessionMigrateResult {
            session,
    ...
    fn host_sync_prefs(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::HostSyncPrefsParams = decode(params)?;
    ...
    fn host_initialize(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::HostInitializeParams = decode(params)?;
        let manager = self
            .remote
            .as_ref()
            .ok_or_else(crate::remote::transport_unavailable)?;
        ...
        let captured = manager
            ...
            .map_err(io_control_error)?;
        encode(&zeus_proto::HostInitializeResult {
    ...
    fn host_list_directories(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::HostListDirectoriesParams = decode(params)?;
        let request = zeus_proto::remote_pty::DirectoryListRequest { path: p.path };
    ...
    fn host_locate_repo(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::HostLocateRepoParams = decode(params)?;
        let target = p
            .host
            .as_deref()
        ...
        let Some(origin) = origin else {
            return encode(&zeus_proto::HostLocateRepoResult {
                path: None,
                origin_url: None,
            });
        };
        ...
        encode(&zeus_proto::HostLocateRepoResult {
            path,
    ...
    fn resolve_host(&self, host_id: &str) -> Result<zeus_proto::HostEntry, ControlError> {
        zeus_proto::HostsConfig::load(self.hosts_file())
            .hosts
            .into_iter()
    ...
    fn session_send_text(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::SendTextParams = decode(params)?;
        let mut registry = self.registry.lock().map_err(poisoned)?;
    ...
    fn session_resize(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::ResizeParams = decode(params)?;
    ...
    fn session_read_screen(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        ...
        encode(&zeus_proto::ReadScreenResult {
    ...
    fn session_read_scrollback_cells(
        ...
        let p: zeus_proto::ReadScrollbackCellsParams = decode(params)?;
        let registry = self.registry.lock().map_err(poisoned)?;
        let session = registry
            .get(&p.session_id.0)
            .ok_or_else(|| ControlError::not_found(p.session_id.0.clone()))?;
    ...
    fn session_remove(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        ...
        self.events.publish(
            zeus_proto::EventName::SESSION_REMOVED,
    ...
    fn session_rename(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::SessionRenameParams = decode(params)?;
        let mut registry = self.registry.lock().map_err(poisoned)?;
        registry
    ...
    fn client_set_active(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::ClientActiveParams = decode(params)?;
    ...
    fn hook_report(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::HookReportParams = decode(params)?;
        let Some(session_id) = p.zeus_session_id else {
            return Ok(json!({}));
        };
    ...
    fn session_resume(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        ...
        let record = {
            ...
            if registry.get(&p.session_id.0).is_some()
                && !matches!(record.status, zeus_proto::SessionStatus::Exited(_))
            {
        ...
        let record = registry
            ...
            .find(|record| record.id.0 == p.session_id.0)
            .ok_or_else(|| ControlError::not_found(p.session_id.0.clone()))?;
        let exited = matches!(record.status, zeus_proto::SessionStatus::Exited(_));
        if registry.get(&p.session_id.0).is_some() {
            if !exited {
    ...
    fn session_resume_from_history(
        ...
        let p: zeus_proto::ResumeFromHistoryParams = decode(params)?;
        let mut registry = self.registry.lock().map_err(poisoned)?;
        let id = next_session_id();
        ...
        if let Some(title) = &p.entry.title {
            record.title = title.clone();
            record.title_source = zeus_proto::TitleSource::FirstPrompt;
        }
    ...
    /// Claude must not silently lose status detection or the zeus tools.
    fn resume_spec(
        &self,
        registry: &Registry,
        id: &str,
    ...
    fn project_add(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::ProjectAddParams = decode(params)?;
        let mut registry = self.registry.lock().map_err(poisoned)?;
    ...
    fn session_read_diff(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::SessionReadDiffParams = decode(params)?;
        let (cwd, host_id) = {
            let registry = self.registry.lock().map_err(poisoned)?;
            registry
                .records()
    ...
    fn governor_configure(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::GovernorSettingsParams = decode(params)?;
    ...
    fn session_hibernate(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        ...
        let mut registry = self.registry.lock().map_err(poisoned)?;
        registry
            .hibernate(&p.session_id.0, zeus_proto::HibernationReason::Manual)
            .map_err(io_control_error)?;
        let _ = registry.persist();
        self.publish_updated(&registry, &p.session_id.0);
        Ok(json!({}))
    ...
    fn daemon_shutdown_if_idle(&self) -> Result<JsonValue, ControlError> {
        ...
        if let Some(reason) = refusal {
            return encode(&zeus_proto::DaemonShutdownIfIdleResult {
                will_exit: false,
        ...
        encode(&zeus_proto::DaemonShutdownIfIdleResult {
            will_exit: true,
            reason: None,
        })
    }
    ...
    fn publish_updated(&self, registry: &Registry, id: &str) {
        ...
        {
            self.events
                .publish_encoded(zeus_proto::EventName::SESSION_UPDATED, &record, Some(id));
        }
    }
    ...
    fn session_history(&self) -> Result<JsonValue, ControlError> {
        ...
        let home = std::env::var("HOME")
            .map(PathBuf::from)
            .map_err(|_| ControlError::internal("HOME is not set"))?;
        let entries: Vec<zeus_proto::HistoryEntry> = crate::history::scan(&home, &tracked)
            .into_iter()
    ...
    fn worktree_list(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::WorktreeListParams = decode(params)?;
    ...
    fn worktree_remove(&self, params: Option<JsonValue>) -> Result<JsonValue, ControlError> {
        let p: zeus_proto::WorktreeRemoveParams = decode(params)?;
...
fn random_session_token() -> Result<zeus_proto::remote_pty::SessionToken, ControlError> {
    ...
    let encoded = bytes
        ...
        .map(|byte| format!("{byte:02x}"))
        .collect::<String>();
    zeus_proto::remote_pty::SessionToken::new(encoded)
        .map_err(|error| ControlError::internal(error.to_string()))
}

pub(crate) fn new_record(id: &str, kind: &str, cwd: &str) -> zeus_proto::SessionRecord {
    use zeus_proto::{AgentKind, DateMillis, Resumability, SessionId, TitleSource};
    let now: DateMillis = std::time::SystemTime::now().into();
    zeus_proto::SessionRecord {
        ...
        status: zeus_proto::SessionStatus::Starting,
        needs_input: None,
...
/// Decodes params into the shared `zeus-proto` type for the method — the same
...
fn history_entry_to_wire(entry: crate::history::HistoryEntry) -> zeus_proto::HistoryEntry {
    zeus_proto::HistoryEntry {
        id: entry.id,
        kind: match entry.kind {
            crate::history::HistoryKind::ClaudeCode => zeus_proto::AgentKind::CLAUDE_CODE,
            crate::history::HistoryKind::Codex => zeus_proto::AgentKind::CODEX,
        },
        cwd: entry.cwd,
        title: entry.title,
        transcript_path: entry.transcript_path,
        last_active_at: zeus_proto::DateMillis::from(entry.last_active_at),
        created_at: entry.created_at.map(zeus_proto::DateMillis::from),
        cwd_exists: entry.cwd_exists,
    }
}

fn worktree_to_wire(info: crate::git::WorktreeInfo) -> zeus_proto::WorktreeInfo {
    zeus_proto::WorktreeInfo {
        path: info.path,
        branch: info.branch,
...
/// Handles the only startup prompt Zeus can safely pre-authorize: the exact
...
fn inject_initial_prompt(registry: &Arc<Mutex<Registry>>, session_id: &str, prompt: &str) {
    ...
    eprintln!(
        "zeusd: {session_id} never accepted its initial prompt within \
...
fn agent_started_working(registry: &Arc<Mutex<Registry>>, session_id: &str) -> bool {
    with_session(registry, session_id, |session| {
        matches!(
            session.view().status,
            zeus_proto::SessionStatus::Working | zeus_proto::SessionStatus::NeedsInput(_)
        )
    })
    .unwrap_or(false)
}
...
mod tests {
    ...
    fn test_record(id: &str) -> zeus_proto::SessionRecord {
        use zeus_proto::*;
        SessionRecord {
    ...
    fn client_activity_drives_pr_monitor_visibility() {
        ...
        let _ = ok_of(call(
            &server,
            zeus_proto::Method::CLIENT_SET_ACTIVE,
    ...
    fn resume_relaunches_a_session_whose_agent_died_on_its_own() {
        ...
        )));
        {
            ...
            record.kind = zeus_proto::AgentKind::new("probe");
    ...
    fn record_mutations_round_trip_over_the_wire() {
        ...
        assert_eq!(
            record["titleSource"],
            serde_json::to_value(zeus_proto::TitleSource::UserRename).expect("encode")
        );
    ...
    fn host_initialization_fails_closed_without_the_remote_transport() {
        let temp = tempfile::tempdir().expect("temp");
        zeus_proto::HostsConfig {
            hosts: vec![zeus_proto::HostEntry {
                id: "forge".into(),
