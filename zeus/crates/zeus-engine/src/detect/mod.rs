...
//! `crates/zeus-engine/manifests`. Adding an agent remains a data-only change,
...
pub use zeus_terminal_state::ScreenSnapshot;

use manifest::Rule;

/// Source-tree location of the Rust-owned built-in Agent catalog. Release
/// packaging copies this directory next to `zeusd-rs`; this fallback keeps
...
mod tests {
    ...
    /// decodes it as `zeus_proto::AgentDescriptor`. That type needs `id` and
    ...
    fn every_shipped_descriptor_decodes_the_way_the_client_decodes_it() {
        ...
        for id in engine.ids() {
            ...
            let descriptor: zeus_proto::AgentDescriptor = serde_json::from_value(raw.clone())
    ...
    fn first_class_agents_ship_verified_official_setup_links() {
        ...
        for id in engine.ids() {
            let descriptor: zeus_proto::AgentDescriptor =
