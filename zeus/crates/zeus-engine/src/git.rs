...
pub fn generated_branch_name() -> String {
    ...
    format!("zeus/{adjective}-{noun}-{hex:04x}")
}
...
const DIFF_RESPONSE_MARKER: &[u8] = b"ZEUS_GIT_V1\0";
...
printf 'ZEUS_GIT_V1\0%s\0%s\0' "$root" "$base_ref"
(
...
pub fn working_diff(
    cwd: &Path,
    base: Option<&zeus_proto::SessionDiffBase>,
) -> std::io::Result<zeus_proto::SessionReadDiffResult> {
...
fn working_diff_input(
    ...
    let comparison = match base {
        Some(zeus_proto::SessionDiffBase::Head) => "head",
        _ => "defaultBranch",
    };
    Ok(format!("{cwd}\n{comparison}\n").into_bytes())
}

fn parse_working_diff(stdout: Vec<u8>) -> std::io::Result<zeus_proto::SessionReadDiffResult> {
    let marker = stdout
    ...
    Ok(zeus_proto::SessionReadDiffResult {
...
mod tests {
    ...
    fn a_generated_branch_name_is_readable_and_namespaced() {
        ...
        assert!(name.starts_with("zeus/"), "got {name}");
        let tail = &name["zeus/".len()..];
    ...
    fn a_branch_becomes_a_usable_directory_slug() {
        assert_eq!(
            branch_to_path_slug("zeus/swift-wren-1a2b"),
            "zeus-swift-wren-1a2b"
        );
        assert_eq!(
    ...
    fn worktrees_can_be_created_listed_and_removed() {
        ...
        assert!(
            created.path.contains("repo-zeus-"),
