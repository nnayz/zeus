...
use zeus_proto::{PortInfo, SessionStatus};
    ...
    for record in &records {
        ...
        if let Some(idle_since) = idle_since(record, attached) {
            if footprint > config.hard_memory_bytes {
                let _ = hibernate(registry, events, &id, zeus_proto::HibernationReason::MemoryPressure);
                continue;
            }
    ...
    if config.idle_threshold_seconds > 0.0 {
        for (id, idle_since, _) in &idle_candidates {
            if (now_ms - idle_since) / 1000.0 > config.idle_threshold_seconds {
                let _ = hibernate(registry, events, id, zeus_proto::HibernationReason::Idle);
            }
        }
    }
        ...
        for (id, idle_since, footprint) in by_oldest {
            ...
            if (now_ms - idle_since) / 1000.0 > config.budget_min_idle_seconds {
                let _ = hibernate(
                    ...
                    events,
                    &id,
                    zeus_proto::HibernationReason::MemoryPressure,
                );
...
fn apply_sample(
    ...
    if let Some(event) = event {
        ...
        events.publish_encoded(zeus_proto::EventName::SESSION_RESOURCES, &event, Some(id));
    }
}
...
fn hibernate(
    ...
    if let Some(record) = record {
        events.publish_encoded(zeus_proto::EventName::SESSION_UPDATED, &record, Some(id));
    }
    Ok(())
}
...
fn idle_since(record: &zeus_proto::SessionRecord, attached: bool) -> Option<f64> {
