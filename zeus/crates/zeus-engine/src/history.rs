...
mod tests {
    ...
    ///
    /// ```sh
    /// ZEUS_INTEROP_HISTORY=1 cargo test -p zeus-engine -- --ignored
    /// ```
    #[test]
    #[ignore = "set ZEUS_INTEROP_HISTORY=1 to scan the real transcript stores"]
    fn reads_the_real_transcript_stores() {
        if std::env::var("ZEUS_INTEROP_HISTORY").is_err() {
            eprintln!("skipped: ZEUS_INTEROP_HISTORY is not set");
            return;
        }
