...
impl HolderLauncher {
    ...
    /// Where the holder binary lives: the `ZEUS_HOLDER_PATH` override, or
    /// next to the running executable.
    pub fn default_executable_path() -> PathBuf {
        if let Ok(configured) = std::env::var("ZEUS_HOLDER_PATH") {
        ...
        let candidates: Vec<PathBuf> = beside.iter().map(|dir| dir.join("zeus-holder")).collect();
        candidates
            .iter()
            ...
            .unwrap_or_else(|| {
                candidates
                    .first()
                    .cloned()
                    .unwrap_or_else(|| PathBuf::from("zeus-holder"))
            })
    }
}
