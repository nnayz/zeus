...
use zeus_proto::{HostEntry, HostSyncPrefsResult, PrefsSyncToolReport};
...
mod tests {
    ...
    fn a_command_substitution_cannot_escape_shell_quote() {
        let quoted = shell_quote("$(touch /tmp/zeus-injection-canary)");
        let echoed = std::process::Command::new("/bin/sh")
            .args(["-c", &format!("printf %s {quoted}")])
            .output()
            .expect("sh");
        assert_eq!(
            String::from_utf8_lossy(&echoed.stdout),
            "$(touch /tmp/zeus-injection-canary)",
            "the substitution must arrive as literal text, not run"
        );
        assert!(
            !std::path::Path::new("/tmp/zeus-injection-canary").exists(),
