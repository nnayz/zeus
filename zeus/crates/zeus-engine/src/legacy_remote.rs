...
//! `origin/main:crates/zeus-engine/src/remote.rs`) is a pure truncation, not a
//! hash: strip the `s_` prefix from the persisted session id and take the first
//! eight characters, so `s_4b99600fd4f1` became the tmux session `zeus-4b99600f`.
//! A target is therefore only ever derived from a record this Engine already
//! owns, and it is additionally required to match `zeus-` plus lowercase hex —
...
//!   prefix and then fnmatch matching, so a bare `-t zeus-4b99600f` could take
//!   out `zeus-4b99600f-something` and a target containing `*` would be a
//!   wildcard. Every target here is sent as `=name`, tmux's documented
//!   exact-match form, and shell-quoted on the way out.
//! * Panes named `zeus-*` that no record refers to are **inventoried and
...
/// a definitive answer.
const SENTINEL: &str = "zeus-legacy ";
...
///
/// Recovered verbatim from `origin/main:crates/zeus-engine/src/remote.rs`, where
...
/// persisted session id so respawning the same zeus session reattached its
...
pub fn legacy_pane_name(session_id: &str) -> String {
    ...
    format!("zeus-{}", raw.chars().take(8).collect::<String>())
}
...
/// killed nor reported.
fn is_zeus_generated_name(name: &str) -> bool {
...
struct Marker {
    ...
    /// `zeus-*` panes seen on a host that no record accounts for, keyed by host
...
pub struct Outcome {
    ...
    /// `zeus-*` panes with no matching record, by host id. **Never killed.**
    ///
...
impl Outcome {
    ...
    pub fn summary(&self) -> Option<String> {
        ...
        if !self.migrated.is_empty() {
            lines.push(format!(
                "zeus-engine: migrated {} pre-Holder remote session(s) to exited+resumable: {:?}",
                self.migrated.len(),
                self.migrated
            ));
        }
        if !self.panes_killed.is_empty() {
            lines.push(format!(
                "zeus-engine: retired {} orphaned remote pane(s) zeus had created: {:?}",
        ...
        if !self.panes_absent.is_empty() {
            lines.push(format!(
                "zeus-engine: {} pre-Holder pane(s) were already gone",
                self.panes_absent.len()
            ));
        }
        if !self.panes_deferred.is_empty() {
            lines.push(format!(
                "zeus-engine: {} pre-Holder pane(s) could not be reached; will retry next launch: {:?}",
        ...
        if !self.panes_given_up.is_empty() {
            lines.push(format!(
                "zeus-engine: giving up on {} unreachable pre-Holder pane(s) after {MAX_PANE_ATTEMPTS} attempts: {:?}",
        ...
        for (host, panes) in &self.orphan_panes {
            lines.push(format!(
                "zeus-engine: {} tmux pane(s) named zeus-* on host {host} match no session record and were NOT touched: {:?}",
                panes.len(),
                panes
            ));
        }
...
/// the pane for anyone who happened to launch zeus while away from the box —
...
pub fn retire_legacy_remote_sessions(plan: &Plan<'_>, run: &ShellRun<'_>) -> Outcome {
    ...
    for (id, host_id) in &legacy {
        ...
        if !is_zeus_generated_name(&target) {
        ...
        match sweep {
            Some(sweep) => {
                ...
                let orphans: Vec<String> = sweep
                    .panes
                    .into_iter()
                    .filter(|pane| is_zeus_generated_name(pane) && !accounted.contains(pane))
                    .collect();
                if !orphans.is_empty() {
...
fn sweep_host(host: &HostEntry, targets: &[String], run: &ShellRun<'_>) -> Option<HostSweep> {
    ...
    let command = format!(
        "if ! command -v tmux >/dev/null 2>&1; then printf 'zeus-legacy no-tmux\\n'; exit 0; fi; \
         tmux list-sessions -F 'zeus-legacy pane #{{session_name}}' 2>/dev/null || true; \
         for target in {list}; do \
         if tmux kill-session -t \"=$target\" >/dev/null 2>&1; then \
         printf 'zeus-legacy killed %s\\n' \"$target\"; \
         else printf 'zeus-legacy absent %s\\n' \"$target\"; fi; \
         done"
    );
...
mod tests {
    ...
    fn the_legacy_name_truncates_the_session_id_and_never_hashes_it() {
        assert_eq!(legacy_pane_name("s_4b99600fd4f1"), "zeus-4b99600f");
        assert_eq!(legacy_pane_name("s_228ca6dbe1a3"), "zeus-228ca6db");
    ...
    fn only_names_this_program_could_have_generated_qualify() {
        assert!(is_zeus_generated_name("zeus-4b99600f"));
        assert!(is_zeus_generated_name("zeus-228ca6db"));
        // A user's own pane that merely starts with the prefix is not ours.
        assert!(!is_zeus_generated_name("zeus-notes"));
        assert!(!is_zeus_generated_name("zeus-"));
        assert!(!is_zeus_generated_name("zeus-4b99600f-extra"));
        assert!(!is_zeus_generated_name("zeus-1"));
        assert!(!is_zeus_generated_name("zeus-4B99600F"));
        assert!(!is_zeus_generated_name("zeus-*"));
    }
    ...
    fn a_host_that_says_nothing_we_recognize_is_not_a_definitive_answer() {
        ...
        let sweep = parse_sweep("zeus-legacy no-tmux\n").expect("sentinel seen");
