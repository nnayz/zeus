...
mod tests {
    ...
    /// `ZEUS_INTEROP_LOG` at a **copy** of one — never at a live session's
    /// file, which its holder is still appending to:
    ///
    /// ```sh
    /// cp "~/Library/Application Support/Zeus/logs/s_xxx.bin" /tmp/probe.bin
    /// ZEUS_INTEROP_LOG=/tmp/probe.bin cargo test -p zeus-engine -- --ignored
    /// ```
    #[test]
    #[ignore = "needs ZEUS_INTEROP_LOG pointing at a copy of a Swift-written log"]
    fn reads_a_log_written_by_the_swift_holder() {
        ...
        let Ok(raw) = std::env::var("ZEUS_INTEROP_LOG") else {
            eprintln!("skipped: ZEUS_INTEROP_LOG is not set");
            return;
        };
