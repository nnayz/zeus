...
pub fn spawn_pr_monitor(
    ...
    std::thread::Builder::new()
        .name("zeus-pr-monitor".into())
        .spawn(move || {
            let Some(gh) = resolve_gh() else {
                eprintln!("zeusd-rs: pull-request monitor idle: gh not on PATH");
                return;
            };
...
mod tests {
    ...
    fn coordinates_come_out_of_a_pr_url() {
        assert_eq!(
            pr_coordinates("https://github.com/nnayz/zeus/pull/7"),
            Some(("nnayz".into(), "zeus".into(), 7))
        );
