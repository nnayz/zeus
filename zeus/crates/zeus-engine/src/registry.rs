...
use zeus_proto::{DateMillis, SessionRecord, SessionStatus, TitleSource};
...
pub fn spawn_persist_flusher(
    ...
    std::thread::Builder::new()
        .name("zeus-persist-flusher".into())
        .spawn(move || {
...
impl Registry {
    ...
    fn fold_live(&self, record: &mut SessionRecord) {
        ...
        if matches!(record.status, SessionStatus::Exited(_))
            && record.resumability == zeus_proto::Resumability::Live
        {
            record.resumability = if self.can_reenter(record) {
                zeus_proto::Resumability::Resumable
            } else {
                zeus_proto::Resumability::NotResumable
            };
        }
    }
    ...
    pub fn terminate(
        ...
        if let Some(record) = self.records.get_mut(id) {
            record.status = SessionStatus::Exited(zeus_proto::ExitInfo {
                reason: match exit {
                    crate::pty::Exit::Signal(_) => zeus_proto::ExitReason::Signaled,
                    crate::pty::Exit::Code(_) => zeus_proto::ExitReason::Exited,
                },
                code: match exit {
    ...
    pub fn apply_hook_metadata(&mut self, id: &str, meta: &crate::hooks::HookMetadata) -> bool {
        let generated_title = self.records.get(id).and_then(|record| {
            let accepts_generated_title = record.kind == zeus_proto::AgentKind::CLAUDE_CODE
                && matches!(
                    record.title_source,
    ...
    pub fn hibernate(
        &mut self,
        id: &str,
        reason: zeus_proto::HibernationReason,
    ) -> std::io::Result<()> {
        let tree = {
    ...
    pub fn apply_resource_sample(
        ...
        ports: Option<Vec<zeus_proto::PortInfo>>,
        artifacts: Option<Vec<zeus_proto::SessionArtifact>>,
    ) -> Option<zeus_proto::SessionResourcesEvent> {
        ...
        Some(zeus_proto::SessionResourcesEvent {
            id: record.id.clone(),
    ...
    pub fn apply_pull_request_statuses(
        &mut self,
        id: &str,
        statuses: Vec<zeus_proto::PullRequestStatus>,
    ) -> bool {
        let Some(record) = self.records.get_mut(id) else {
            return false;
        };
    ...
    pub fn set_hibernation(&mut self, id: &str, info: Option<zeus_proto::HibernationInfo>) {
        if let Some(record) = self.records.get_mut(id) {
    ...
    pub fn archive(&mut self, id: &str) -> std::io::Result<()> {
        ...
        if !matches!(record.status, SessionStatus::Exited(_)) {
            record.status = SessionStatus::Exited(zeus_proto::ExitInfo {
                reason: zeus_proto::ExitReason::Archived,
                code: None,
                signal: None,
            });
        }
...
fn fold_session_view(record: &mut SessionRecord, view: &SessionView) {
    ...
    if record.kind == zeus_proto::AgentKind::SHELL
        || matches!(
            record.title_source,
            TitleSource::AgentProvided | TitleSource::ZeusAssigned | TitleSource::UserRename
        )
    {
        return;
    }
...
/// titles by older builds. User and Zeus-assigned names are intentionally
...
pub(crate) fn session_project_id(root: &str, host: Option<&str>) -> zeus_proto::ProjectId {
    let location = host.map_or_else(|| root.to_owned(), |host| format!("ssh\0{host}\0{root}"));
    zeus_proto::ProjectId(project_id(&location))
}
...
mod tests {
    ...
    fn a_conversation_that_outlived_its_session_reports_resumable() {
        ...
        dead.status = SessionStatus::Exited(zeus_proto::ExitInfo {
    ...
    /// `ZEUS_INTEROP_STATE` at a **copy** — never at the live file, which the
    /// running daemon rewrites:
    ///
    /// ```sh
    /// cp "~/Library/Application Support/Zeus/state.json" /tmp/state.json
    /// ZEUS_INTEROP_STATE=/tmp/state.json cargo test -p zeus-engine -- --ignored
    /// ```
    #[test]
    #[ignore = "needs ZEUS_INTEROP_STATE pointing at a copy of a Swift-written state.json"]
    fn reads_the_state_file_the_swift_daemon_wrote() {
        let Ok(raw) = std::env::var("ZEUS_INTEROP_STATE") else {
            eprintln!("skipped: ZEUS_INTEROP_STATE is not set");
            return;
        };
    ...
    fn pty_titles_are_filtered_fallbacks_and_never_override_user_renames() {
        ...
        generic.cwd = "/work/zeus".to_owned();
        let generic_view = SessionView {
            title: Some("zeus".to_owned()),
            ..view
        };
