...
pub const PLATFORM_PROBE_COMMAND: &str =
    "printf '__ZEUS_PLATFORM_V1__\\0%s\\0%s\\0%s\\0' \"$(uname -s)\" \"$(uname -m)\" \"$HOME\"";
...
impl RemoteInstallLayout {
    ...
    pub fn executable(&self) -> String {
        format!("{}/zeus-remote", self.version_dir())
    }
...
mod tests {
    ...
    fn noisy_platform_probe_maps_all_supported_targets() {
        for (os, arch, target) in [
            ...
            let output = format!("banner\n__ZEUS_PLATFORM_V1__\0{os}\0{arch}\0/home/dev\0noise");
            assert_eq!(
    ...
    fn unsupported_and_malformed_platforms_fail_closed() {
        ...
        assert!(matches!(
            PlatformProbe::parse(b"__ZEUS_PLATFORM_V1__\0FreeBSD\0x86_64\0/home/u\0"),
            Err(BootstrapError::UnsupportedPlatform { .. })
        ));
        assert!(matches!(
            PlatformProbe::parse(b"__ZEUS_PLATFORM_V1__\0Darwin\0x86_64\0/Users/u\0"),
            Err(BootstrapError::UnsupportedPlatform { .. })
        ));
        assert_eq!(
        ...
        assert!(matches!(
            PlatformProbe::parse(b"__ZEUS_PLATFORM_V1__\0Linux\0x86_64\0relative\0"),
    ...
    fn packaged_artifact_is_verified_before_upload() {
        ...
        let path = temp.path().join("zeus-remote");
    ...
    fn install_layout_is_versioned_and_nonce_scoped() {
        ...
        assert_eq!(
            layout.executable(),
            "$HOME/.cache/zeus/bin/protocol-1/build-1/zeus-remote"
        );
        assert_eq!(
            layout.temporary(),
            "$HOME/.cache/zeus/bin/protocol-1/build-1/.tmp-nonce_2"
        );
