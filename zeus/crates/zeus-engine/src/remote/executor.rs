...
impl ProcessExecutor {
    ...
    /// Configure Zeus's UI broker for OpenSSH password, key-passphrase and
    ...
    fn command(&self, spec: &CommandSpec) -> Command {
        ...
        if let Some(askpass) = &self.askpass_executable {
            ...
            if std::env::var_os("DISPLAY").is_none() {
                command.env("DISPLAY", "zeus-native-askpass");
            }
        }
        command
    }
...
mod tests {
    ...
    fn askpass_is_forced_without_consuming_protocol_stdin() {
        let executor = ProcessExecutor::new("/bin/sh").with_askpass("/tmp/zeus-askpass");
        let spec = CommandSpec {
            program: "ignored".into(),
            arguments: vec![
                "-c".into(),
        ...
        assert!(fields.starts_with("/tmp/zeus-askpass|force|"));
