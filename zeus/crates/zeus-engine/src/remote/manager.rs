...
// These wall-clock bounds include time spent in Zeus's native OpenSSH
...
fn normalized_control_dir(requested: &Path) -> PathBuf {
    ...
    PathBuf::from(format!("/tmp/zeus-ssh-{}-{suffix}", effective_uid()))
}
...
mod tests {
    ...
    fn helper_probe_without_directory_management_is_rejected_before_rpc() {
        ...
        let artifact = PackagedArtifact {
            ...
            protocol_major: zeus_proto::remote_pty::PROTOCOL_MAJOR,
        ...
        let probe = HelperProbe {
            ...
            capabilities: PHASE_ONE_HELPER_CAPABILITIES
                ...
                .copied()
                .filter(|capability| {
                    *capability != zeus_proto::remote_pty::RemoteCapability::DirectoryList
                })
                .collect(),
        };
    ...
    fn fake_ssh_bootstrap_uploads_activates_and_then_reuses_exact_build() {
        ...
        writeln!(
            fake,
            "#!/bin/sh\nexport HOME='{}'\nexport TEST_ARTIFACT_SHA='{}'\nfor last; do :; done\ncase \"$last\" in *'cat >'*) printf 'upload\\n' >> '{}';; esac\ncase \"$last\" in\n  *'__ZEUS_PLATFORM_V1__'*) printf '__ZEUS_PLATFORM_V1__\\0Darwin\\0aarch64\\0%s\\0' \"$HOME\";;\n  *) exec /bin/sh -c \"$last\";;\nesac",
            remote_home.display(),
            artifact_sha,
            upload_log.display()
        )
        ...
        let final_path = remote_home.join(".cache/zeus/bin/protocol-1/test-build/zeus-remote");
