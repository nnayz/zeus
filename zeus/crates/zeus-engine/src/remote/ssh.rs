...
impl SshTransport {
    ...
    /// Requests shutdown of Zeus's private multiplexing master. This affects
    ...
    pub fn helper_probe(&self, build_id: &str) -> Result<CommandSpec, BootstrapError> {
        ...
        Ok(self.channel(&format!(
            "set -eu; [ -d \"$HOME/.cache\" ] && [ ! -L \"$HOME/.cache\" ] || exit 73; exec \"$HOME/.cache/zeus/bin/protocol-{}/{}/zeus-remote\" probe --format=json",
            zeus_proto::remote_pty::PROTOCOL_MAJOR,
            build_id
        )))
    }
    ...
    pub fn helper_command(
        ...
        validate_component("build id", build_id)?;
        Ok(self.channel(&format!(
            "set -eu; [ -d \"$HOME/.cache\" ] && [ ! -L \"$HOME/.cache\" ] || exit 73; exec \"$HOME/.cache/zeus/bin/protocol-{}/{}/zeus-remote\" {}",
            zeus_proto::remote_pty::PROTOCOL_MAJOR,
            build_id,
            command.as_str()
        )))
...
/// shell may be fish, csh, or another non-POSIX shell, while Zeus's fixed
...
mod tests {
    ...
    fn every_protocol_channel_disables_ssh_pty_allocation() {
        let transport = SshTransport::new(&host(), "/tmp/zeus master/socket");
        ...
        assert!(words.contains(&"ControlPath=/tmp/zeus master/socket".to_string()));
    ...
    #[test]
    fn control_exit_targets_only_the_zeus_mux_path() {
        let transport = SshTransport::new(&host(), "/tmp/zeus master/socket");
    ...
    fn helper_path_accepts_only_validated_build_ids() {
        ...
        assert!(
            words
                .last()
                .expect("command")
                .contains("protocol-1/build-123/zeus-remote")
        );
    ...
    fn upload_is_owner_only_symlink_checked_and_nonce_scoped() {
        ...
        assert!(!cleanup.contains("zeus-remote"));
    }
