...
use zeus_proto::{NeedsInputDetail, SessionStatus};
use zeus_terminal_state::GridMirror;
...
fn replay_budget() -> usize {
    std::env::var("ZEUS_REPLAY_BUDGET_BYTES")
        .ok()
...
pub struct SessionView {
    ...
    pub title_source: Option<zeus_proto::TitleSource>,
    pub tail_offset: u64,
    pub exited: bool,
}
...
struct Shared {
    ...
    artifacts: Mutex<Vec<zeus_proto::SessionArtifact>>,
...
struct RemoteGridState {
    mirror: GridMirror,
    revision: u64,
    pending: Option<zeus_proto::grid::GridUpdate>,
}
...
/// public [`zeus_proto::SessionRecord`].
#[derive(Clone)]
...
pub struct RemoteAdoptSpec {
    pub manager: Arc<RemoteManager>,
    pub helper: InstalledHelper,
    pub token: zeus_proto::remote_pty::SessionToken,
...
struct RemoteLaunchCleanup {
    ...
    selector: zeus_proto::remote_pty::SessionSelector,
    armed: bool,
}
...
impl Session {
        ...
        *shared.remote_grid.lock().expect("remote grid") = Some(RemoteGridState {
        ...
        let pump = {
            ...
            std::thread::Builder::new()
                .name(format!("zeus-remote-session-{}", spec.id))
    ...
    fn spawn_direct(spec: SessionSpec, engine: Arc<ManifestEngine>) -> std::io::Result<Self> {
        ...
        let pump = {
            ...
            let manifest_id = spec.manifest_id.clone();
            std::thread::Builder::new()
                .name(format!("zeus-session-{}", spec.id))
    ...
    pub fn view(&self) -> SessionView {
        ...
        let (title, title_source) = if let Some(title) = prompt_title {
            (Some(title), Some(zeus_proto::TitleSource::FirstPrompt))
        } else {
            (
                self.shared.title.lock().expect("title").clone(),
                Some(zeus_proto::TitleSource::AgentProvided),
            )
        };
        SessionView {
    ...
    pub fn artifacts(&self) -> Vec<zeus_proto::SessionArtifact> {
    ...
    ) -> Option<zeus_proto::grid::GridUpdate> {
        if let Some(remote) = self
            .shared
            .remote_grid
            .lock()
    ...
    pub fn read_scrollback(&self) -> zeus_proto::ReadScrollbackResult {
    ...
    pub fn read_scrollback_cells(
        ...
        first_row: i64,
        max_rows: i64,
    ) -> zeus_proto::ReadScrollbackCellsResult {
...
fn scan_artifacts_if_due(
    ...
    *last_scan_seq = seq;
    ...
    let now = zeus_proto::DateMillis::from(SystemTime::now());
