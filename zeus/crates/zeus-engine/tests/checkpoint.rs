...
use zeus_engine::checkpoint::ScreenCheckpoint;
use zeus_engine::session::HolderConfig;
use zeus_engine::{ManifestEngine, OutputLog, Registry};
...
fn holders_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("zeus-ckpt-{tag}-{}", std::process::id()));
...
) -> zeus_engine::session::SessionSpec {
    zeus_engine::session::SessionSpec {
        id: id.into(),
        pty: zeus_engine::PtySpec::new(vec!["/bin/sh".into(), "-c".into(), script.into()], "/tmp")
            .env("PATH", "/usr/bin:/bin")
            .env("TERM", "xterm-256color")
            .size(80, 24),
        manifest_id: "shell".into(),
        authority: zeus_engine::Authority::ProcessOnly,
