...
fn holders_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("zeus-hold-{tag}-{}", std::process::id()));
...
fn the_launcher_bootstraps_a_real_manager_process() {
    ...
    let logs = root.join("logs");
    let binary = PathBuf::from(env!("CARGO_BIN_EXE_zeus-holder"));
    ...
    unsafe { std::env::set_var("ZEUS_HOLDER_IDLE_SECONDS", "1") };
