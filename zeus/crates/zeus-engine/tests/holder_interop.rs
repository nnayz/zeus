...
//! exit-marker parser against the REAL Swift `zeusd-holder` binary.
//!
...
//!
//! ```sh
//! cd ../.. && swift build --product zeusd-holder
//! ```
...
use zeus_engine::OutputLog;
use zeus_engine::holder::protocol::DEFAULT_DISK_CAPACITY;
use zeus_engine::holder::{
...
fn swift_holder() -> Option<PathBuf> {
    let candidate = Path::new(env!("CARGO_MANIFEST_DIR")).join("../../../.build/debug/zeusd-holder");
    match candidate.canonicalize() {
        Ok(path) => Some(path),
        Err(_) => {
            eprintln!(
                "SKIPPED: Swift zeusd-holder not built at {}; \
                 run `swift build --product zeusd-holder` in the outer repo",
                candidate.display()
            );
            None
        }
...
fn holders_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("zeus-interop-{tag}-{}", std::process::id()));
