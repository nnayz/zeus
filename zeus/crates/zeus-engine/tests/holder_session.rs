...
use zeus_engine::holder::{HolderClient, HolderPaths};
use zeus_engine::session::{HolderConfig, Session, SessionSpec};
use zeus_engine::{Authority, ManifestEngine, OutputLog, PtySpec, Registry};
use zeus_proto::SessionStatus;
...
fn holders_dir(tag: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("zeus-hsess-{tag}-{}", std::process::id()));
...
fn a_held_session_survives_its_session_object_and_is_adoptable() {
    ...
    let exit = adopted
        ...
        .expect("terminate");
    assert!(
        matches!(exit, zeus_engine::Exit::Signal(_)),
...
fn record(id: &str) -> zeus_proto::SessionRecord {
    use zeus_proto::*;
    SessionRecord {
