...
use zeus_engine::status::{Authority, StatusReducer, StatusSignal};
use zeus_engine::{HeadlessScreen, OutputLog, Pty, PtySpec};
...
fn pump_until(
    reader: &mut zeus_engine::pty::PtyStream,
...
"#;
    ...
    for step in 1..=4 {
        ...
        let signal = match observation {
            ...
            None => StatusSignal::Screen(zeus_engine::detect::ScreenObservation {
                state: zeus_engine::detect::ManifestState::Idle,
...
fn a_process_only_agent_reports_working_then_its_exit_code() {
    ...
    let outcome = reducer.reduce(
        StatusSignal::ProcessExit {
            code: match exit {
                zeus_engine::Exit::Code(code) => Some(code),
                zeus_engine::Exit::Signal(_) => None,
            },
            signal: match exit {
                zeus_engine::Exit::Signal(signal) => Some(signal),
                zeus_engine::Exit::Code(_) => None,
            },
        },
        SystemTime::now(),
    );
