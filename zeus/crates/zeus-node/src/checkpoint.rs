    ...
    pub fn stage(
        ...
        let result = (|| {
            ...
            if let Some(provider_state) = &manifest.provider_state {
                self.restore_file(&temporary.join(".zeus"), provider_state)?;
            }
...
fn provider_state_destination(
    ...
    Ok(Some(match manifest.provider {
        ProviderKind::Claude => config_home
            .join("projects/-zeus-imports")
            .join(format!("{session_id}.jsonl")),
        ProviderKind::Codex => config_home
            .join("sessions/zeus-imports")
            .join(format!("rollout-zeus-{session_id}.jsonl")),
    }))
}
...
mod tests {
    ...
    fn staging_installs_provider_state_inside_the_target_profile_home() {
        ...
        assert!(
            target_home
                .join("sessions/zeus-imports/rollout-zeus-thread-1.jsonl")
                .is_file()
        );
    }
}
