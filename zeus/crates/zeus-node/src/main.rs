...
use zeus_client::{NodeClient, NodeClientConfig};
use zeus_node::{NodeConfig, NodePaths, NodeServer, NodeService};
use zeus_proto::{
...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let listen =
        take_option(&mut arguments, "--listen").or_else(|| env::var("ZEUS_NODE_LISTEN").ok());
...
fn print_init(paths: &NodePaths, config: &NodeConfig) {
    ...
    println!("Keep the token private; copy it only to an enrolled Zeus client.");
}
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if let Err(error) = maintenance.refresh_usage().await {
                eprintln!("zeus-node usage refresh failed: {error}");
            }
        }
    });
    ...
    match arguments.first().map(String::as_str) {
        Some("list") => {
            ...
            if catalog.profiles.is_empty() {
                println!("No account profiles. Add one with `zeus-node account add`.");
            }
...
fn local_client(config: &NodeConfig, listen: Option<String>) -> NodeClient {
    NodeClient::new(NodeClientConfig {
        ...
        build: zeus_node::NODE_BUILD.into(),
    })
}
...
fn print_help() {
    println!(
        r#"zeus-node — first-party Zeus execution node

Usage:
  zeus-node init [--home PATH]
  zeus-node status [--home PATH]
  zeus-node serve [--home PATH] [--listen IP:PORT]
  zeus-node account list
  zeus-node account add --provider claude|codex --id ID [--label LABEL]
  zeus-node account default --provider claude|codex --id ID
  zeus-node account status --id ID
  zeus-node account login --id ID [--mode device|browser|interactive]
  zeus-node handoff --provider claude|codex --profile ID --session ID \
