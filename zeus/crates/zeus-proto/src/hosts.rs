...
//! `Sources/ZeusProtocol/HostsConfig.swift`.
//!
//! The file lives next to `remote.json` in the daemon's Application Support
//! directory and is read by both the daemon (at spawn time) and the zeus UI
        ...
        let json = br#"{
            "hosts": [
                { "id": "forge", "name": "Forge", "ssh": "cristi@forge", "defaultCwd": "~/code",
                  "node": { "endpoint": "tcp://100.64.0.2:7337", "tokenFile": "~/.config/zeus/forge.token", "nodeId": "node-forge" } },
        ...
        let config = HostsConfig {
            hosts: vec![HostEntry {
                ...
                node: Some(HostNodeConfig {
                    ...
                    token_file: "~/.config/zeus/forge.token".into(),
