...
fn additive_unknown_variants_fall_back_and_unknown_fields_are_ignored() {
    ...
    let worktree: zeus_proto::WorktreeInfo = serde_json::from_value(json!({
        "path": "/tmp/wt",
        "branch": "main",
        "isBare": false,
        "isDetached": false,
...
fn migration_and_host_methods_use_the_swift_wire_names() {
    ...
    let to_local = zeus_proto::SessionMigrateParams {
        session_id: zeus_proto::SessionId::new("s_1"),
        target_host: None,
    };
    assert_eq!(
    ...
    let to_forge: zeus_proto::SessionMigrateParams =
    ...
    let report: zeus_proto::HostSyncPrefsResult = serde_json::from_value(json!({
        "tools": [
    ...
    let locate = zeus_proto::HostLocateRepoParams {
        host: Some("forge".into()),
        origin_url: None,
        session_id: Some(zeus_proto::SessionId::new("s_1")),
    };
    assert_eq!(
    ...
    let preserving = zeus_proto::SessionSpawnParams {
        same_repo_as: Some(zeus_proto::SessionId::new("s_ref")),
        ..legacy
    };
    assert_eq!(
