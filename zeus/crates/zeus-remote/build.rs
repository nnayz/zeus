...
fn main() {
    println!("cargo:rerun-if-env-changed=ZEUS_REMOTE_BUILD_ID");
    let build_id = std::env::var("ZEUS_REMOTE_BUILD_ID").unwrap_or_else(|_| source_build_id());
    assert!(
        !build_id.is_empty()
            && build_id.len() <= 128
            && build_id
        ...
        "ZEUS_REMOTE_BUILD_ID must be a safe version path component"
    );
    println!("cargo:rustc-env=ZEUS_REMOTE_EFFECTIVE_BUILD_ID={build_id}");
}
...
fn source_build_id() -> String {
    ...
    let roots = [
        ...
        workspace.join("crates/zeus-proto/Cargo.toml"),
        workspace.join("crates/zeus-proto/src"),
        workspace.join("crates/zeus-pty/Cargo.toml"),
        workspace.join("crates/zeus-pty/src"),
        workspace.join("crates/zeus-terminal-state/Cargo.toml"),
        workspace.join("crates/zeus-terminal-state/src"),
    ];
