...
const MARKER: &[u8] = b"ZEUSENV1\0";
const LENGTH_BYTES: usize = 8;
const PAYLOAD_LIMIT: usize = MARKER.len() + LENGTH_BYTES + MAX_LAUNCH_BYTES;
const DIAGNOSTIC_LIMIT: usize = 64 * 1024;
const LOGIN_COMMAND: &str = "exec \"$ZEUS_REMOTE_SELF\" __dump-environment";
const WORKING_DIRECTORY_COMMAND: &str =
    "cd -- \"$ZEUS_REMOTE_CWD\" && exec \"$ZEUS_REMOTE_SELF\" __dump-environment";
...
fn capture_layer(
    ...
    if let Some(cwd) = target_cwd {
        command.env("ZEUS_REMOTE_CWD", cwd);
    }
    command.env("ZEUS_REMOTE_SELF", executable);
    command.env("ZEUS_ENV_CAPTURE", "1");
...
pub fn dump(stdout: &mut dyn Write) -> io::Result<()> {
    if std::env::var("ZEUS_ENV_CAPTURE").ok().as_deref() != Some("1") {
        return Err(io::Error::new(
