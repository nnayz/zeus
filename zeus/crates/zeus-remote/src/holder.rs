...
pub const PHASE_ONE_CAPABILITIES: &[zeus_proto::remote_pty::RemoteCapability] =
...
impl Holder {
    ...
    fn handshake(&mut self, connection: &mut Connection, hello: Hello) -> io::Result<()> {
        if hello.protocol.major != zeus_proto::remote_pty::PROTOCOL_MAJOR {
            return Err(io::Error::new(
                io::ErrorKind::InvalidData,
...
fn resolve_remote_executable(
    argv: &[String],
    environment: &[zeus_proto::remote_pty::EnvironmentVariable],
