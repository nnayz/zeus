...
pub const BUILD_ID: &str = env!("ZEUS_REMOTE_EFFECTIVE_BUILD_ID");
...
pub const HELPER_CAPABILITIES: &[zeus_proto::remote_pty::RemoteCapability] =
...
const HELP: &str = "zeus-remote <command>\n\n\
Commands:\n  \
...
pub fn execute<W: Write + Send>(
    ...
    let invocation = match parse(arguments) {
        ...
        Err(message) => {
            let _ = writeln!(stderr, "zeus-remote: {message}");
            return EXIT_USAGE;
        }
    };
    ...
    let result = match invocation {
        ...
        Invocation::Version => writeln!(stdout, "zeus-remote {BUILD_ID}"),
        ...
        Invocation::Launch => holder::read_limited_json::<_, LaunchRequest>(
            stdin,
            zeus_proto::remote_pty::MAX_LAUNCH_BYTES,
        )
    ...
    match result.and_then(|()| stdout.flush()) {
        ...
        Err(error) => {
            ...
            let _ = writeln!(stderr, "zeus-remote: {error}");
            EXIT_FAILURE
        }
    }
}
...
fn kill(selector: &SessionSelector) -> io::Result<SessionInspection> {
    ...
    if session.persistence == zeus_proto::remote_pty::PersistenceCapability::UserSupervisor {
...
fn gc_helper_builds(referenced: &HashSet<String>) -> io::Result<(usize, usize)> {
    ...
    for entry in entries {
        ...
        let helper = entry.path().join("zeus-remote");
...
fn activate(executable: &Path) -> io::Result<HelperProbe> {
    ...
    let final_path = build_dir.join("zeus-remote");
...
fn validate_activation_path(path: &Path) -> io::Result<()> {
    ...
    // The bootstrap-owned suffix is `.cache/zeus/bin/protocol/build/file`.
    // HOME itself may legitimately be reached through an OS-managed symlink;
    // every component Zeus creates must be a real directory/file.
    for _ in 0..6 {
...
mod tests {
    ...
    fn json_probe_reports_the_exact_artifact_hash_and_holder_capabilities() {
        ...
        let executable = temp.path().join("zeus-remote");
    ...
    fn activation_is_no_replace_and_concurrent_same_bytes_are_idempotent() {
        ...
        assert_eq!(
            fs::read(build.join("zeus-remote")).expect("final"),
            b"same artifact"
        );
        assert!(!first.exists());
