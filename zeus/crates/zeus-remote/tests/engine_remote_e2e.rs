...
use zeus_proto::{HostEntry, SessionStatus};
...
fn engine_bootstraps_detaches_and_adopts_the_same_remote_process() {
    ...
    assert!(matches!(
        after.process_state,
        zeus_proto::remote_pty::RemoteProcessState::Running { pid } if pid == process_pid
    ));
...
fn interrupted_upload_cleans_only_its_nonce_and_is_retryable() {
    ...
    assert!(
        remote_home
            .join(format!(
                ".cache/zeus/bin/protocol-1/{}/zeus-remote",
                installed.build_id
            ))
            .is_file()
    );
