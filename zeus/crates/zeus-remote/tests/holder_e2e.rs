...
fn run_json<T: serde::Serialize, R: serde::de::DeserializeOwned>(
    ...
    let mut child = Command::new(helper())
        .arg(command)
        .env("ZEUS_REMOTE_STATE_DIR", state_dir)
    ...
    messages
        ...
        .map(|message| match message {
            RemoteMessage::Terminal(frame) => match frame.frame_type {
                zeus_proto::frames::FrameType::Output => "Output",
                zeus_proto::frames::FrameType::ReplayBegin => "ReplayBegin",
                zeus_proto::frames::FrameType::ReplayEnd => "ReplayEnd",
                _ => "Terminal",
            },
...
fn list_kill_and_gc_complete_the_session_lifecycle() {
    ...
    let fixture_home = state_dir
        ...
        .expect("state parent")
        .join("fixture-home");
    let stale_build = fixture_home.join(".cache/zeus/bin/protocol-1/stale-build");
    std::fs::create_dir_all(&stale_build).expect("stale build dir");
    std::fs::write(stale_build.join("zeus-remote"), b"stale").expect("stale helper");
...
fn signal_exit_and_holder_failure_are_reported_without_orphaning_the_agent() {
    ...
    let epoch = handshake
        ...
        .expect("epoch");
    attach.send(RemoteMessage::Signal(zeus_proto::remote_pty::Signal {
...
fn grid_text(grid: &zeus_proto::grid::GridUpdate) -> String {
...
fn environment_capture_survives_an_interactive_login_bash() {
    ...
    assert!(captured.environment.iter().all(|variable| {
        !variable.name.starts_with("SSH_") && !variable.name.starts_with("ZEUS_")
    }));
}
