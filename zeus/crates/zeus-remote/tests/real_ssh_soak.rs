...
use zeus_engine::remote::binding::RemoteBindingStore;
use zeus_engine::remote::executor::ProcessExecutor;
use zeus_engine::remote::manager::{ArtifactCatalog, InstalledHelper, RemoteManager};
use zeus_engine::{
...
#[test]
#[ignore = "requires ZEUS_REMOTE_SSH_TARGET and a disposable real SSH account"]
fn real_ssh_detach_soak_reconnects_the_same_process() {
    let target = required_env("ZEUS_REMOTE_SSH_TARGET");
    let helper_path = env::var_os("ZEUS_REMOTE_HELPER_PATH")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(env!("CARGO_BIN_EXE_zeus-remote")));
    let ssh = env::var_os("ZEUS_REMOTE_SSH_EXECUTABLE").unwrap_or_else(|| "ssh".into());
    let requested_cwd = env::var("ZEUS_REMOTE_CWD").unwrap_or_else(|_| "~".into());
    let soak = env::var("ZEUS_REMOTE_SOAK_SECONDS")
        .ok()
...
fn load_binding(
    ...
    session_id: &str,
) -> zeus_engine::remote::binding::RemoteBinding {
    store
        .load_all()
