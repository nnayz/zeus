...
impl TerminalElement {
    ...
    pub fn complete_scrollback_fetch(
        &self,
        result: zeus_proto::methods::ReadScrollbackCellsResult,
        visible_rows: usize,
...
fn append_background_quads(
    ...
    while col < row.len() {
        let cell = row[col];
        let inverse = cell.style.contains(zeus_proto::grid::TermStyle::INVERSE);
        ...
        while end < row.len() {
            let next = row[end];
            let next_inverse = next.style.contains(zeus_proto::grid::TermStyle::INVERSE);
