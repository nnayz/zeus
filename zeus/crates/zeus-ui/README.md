...
Shared GPUI tokens and components ported from Zeus's Swift design system:
...
```sh
cargo run -p zeus-ui --example gallery
```

## Adding an icon

Ordinary interface icons belong to the shared **Zeus Line** SVG family. Do not
...
4. Verify the asset and inspect it alongside the full family from the `zeus/`
   workspace root:

   ```sh
   xmllint --noout crates/zeus-ui/assets/icons/*.svg
   cargo test -p zeus-ui
   cargo check -p zeus-app
   cargo run -p zeus-ui --example gallery
   ```
