...
pub const GROK_PATH: &str = "M9.27 15.29l7.978-5.897c.391-.29.95-.177 1.137.272.98 2.369.542 5.215-1.41 7.169-1.951 1.954-4.667 2.382-7.149 1.406l-2.711 1.257c3.889 2.661 8.611 2.003 11.562-.953 2.341-2.344 3.066-5.539 2.388-8.42l.006.007c-.983-4.232.242-5.924 2.75-9.383.06-.082.12-.164.179-.248l-3.301 3.305v-.01L9.267 15.292M7.623 16.723c-2.792-2.67-2.31-6.801.071-9.184 1.761-1.763 4.647-2.483 7.166-1.425l2.705-1.25a7.808 7.808 0 00-1.829-1A8.975 8.975 0 005.984 5.83c-2.533 2.536-3.33 6.436-1.962 9.764 1.022 2.487-.653 4.246-2.34 6.022-.599.63-1.199 1.259-1.682 1.925l7.62-6.815";
pub const OPENCODE_PATH: &str = "M4 2h16v20H4zM8 6v12h8V6z";
...
impl AgentKind {
    ...
    pub const fn brand_mark(self) -> Option<BrandMarkKind> {
        match self {
            ...
            Self::Grok => Some(BrandMarkKind::Grok),
            Self::OpenCode => Some(BrandMarkKind::OpenCode),
            Self::Shell | Self::Generic => None,
        }
    }

    /// Maps a manifest / protocol agent id onto the hand-drawn brand set.
    /// Unknown ids keep the generic terminal treatment.
...
impl BrandMarkKind {
    ...
    pub const fn label(self) -> &'static str {
        match self {
            ...
            Self::Grok => "Grok slash",
            Self::OpenCode => "OpenCode frame",
        }
    }
    ...
    fn parsed(self) -> &'static SvgPath {
        ...
        static GROK: OnceLock<SvgPath> = OnceLock::new();
        static OPENCODE: OnceLock<SvgPath> = OnceLock::new();
        let cell = match self {
            Self::Claude => &CLAUDE,
            Self::OpenAi => &OPENAI,
            Self::Cursor => &CURSOR,
...
impl RenderOnce for AgentLogo {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        let inset = if self.badged { 0.28 } else { 0.08 };
        let mark_size = self.size * (1.0 - 2.0 * inset);
        let fill = match self.kind {
            AgentKind::ClaudeCode => Palette::CLAY,
            AgentKind::Codex | AgentKind::Cursor | AgentKind::Grok | AgentKind::OpenCode => {
                self.colors.primary.alpha(0.82)
            }
...
mod tests {
    ...
    fn brand_mapping_matches_swift() {
        ...
        assert_eq!(AgentKind::Grok.brand_mark(), Some(BrandMarkKind::Grok));
        assert_eq!(
            AgentKind::OpenCode.brand_mark(),
            Some(BrandMarkKind::OpenCode)
        );
        assert_eq!(AgentKind::Shell.brand_mark(), None);
    }

    #[test]
    fn from_id_covers_first_class_agents() {
        assert_eq!(AgentKind::from_id("claude-code"), AgentKind::ClaudeCode);
        assert_eq!(AgentKind::from_id("codex"), AgentKind::Codex);
        assert_eq!(AgentKind::from_id("cursor"), AgentKind::Cursor);
        assert_eq!(AgentKind::from_id("grok"), AgentKind::Grok);
        assert_eq!(AgentKind::from_id("opencode"), AgentKind::OpenCode);
        assert_eq!(AgentKind::from_id("mystery-cli"), AgentKind::Generic);
    }
}
