...
/// Walks `…/zeus.app/Contents/MacOS/zeus` back up to `…/zeus.app`.
///
...
fn directory_is_writable(directory: &Path) -> bool {
    let probe = directory.join(format!(".zeus-update-probe-{}", std::process::id()));
...
mod tests {
    ...
    fn finds_the_app_above_a_bundled_executable() {
        let bundle = enclosing_bundle(Path::new("/Applications/zeus.app/Contents/MacOS/zeus"));
        assert_eq!(bundle, Some(PathBuf::from("/Applications/zeus.app")));
    }

    #[test]
    fn a_bare_binary_has_no_bundle() {
        assert!(
            enclosing_bundle(Path::new("/Users/giga/fun/zeus/zeus/target/debug/zeus")).is_none()
        );
        assert!(enclosing_bundle(Path::new("/usr/local/bin/zeus")).is_none());
        // Right depth, wrong layout.
        assert!(enclosing_bundle(Path::new("/opt/zeus/Contents/MacOS/zeus")).is_none());
    }
    ...
    fn a_temp_directory_is_writable_and_a_system_one_is_not() {
        ...
        let bundle = directory.path().join("zeus.app");
