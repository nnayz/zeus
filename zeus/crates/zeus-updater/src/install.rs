...
//! swap runs from a detached `/bin/sh` helper that waits for zeus to exit
...
/// Seconds the helper waits for zeus to exit before giving up untouched.
...
pub fn installer_script(pid: u32, staged_app: &Path, target: &Path) -> String {
    ...
    format!(
        r#"#!/bin/sh
# Generated by zeus's updater. Swaps the app bundle once zeus has exited.
set -u
...
while kill -0 "$pid" 2>/dev/null; do
    ...
    if [ "$waited" -gt {attempts} ]; then
        echo "zeus (pid $pid) is still running after {EXIT_GRACE_SECONDS}s; not touching $target" >&2
        exit 1
    fi
    sleep 0.1
done

backup="$target.zeus-previous"
rm -rf "$backup"
...
mod tests {
    ...
    fn script() -> String {
        installer_script(
            4242,
            Path::new("/Users/giga/Library/Caches/zeus/updates/0.2.0/zeus.app"),
            Path::new("/Applications/zeus.app"),
        )
    }
    ...
    fn quotes_paths_that_contain_spaces_and_quotes() {
        let script = installer_script(
            1,
            Path::new("/tmp/it's here/zeus.app"),
            Path::new("/Applications/My Apps/zeus.app"),
        );
        assert!(script.contains(r"staged='/tmp/it'\''s here/zeus.app'"));
        assert!(script.contains("target='/Applications/My Apps/zeus.app'"));
    }
    ...
    fn fake_bundle(root: &Path, name: &str, marker: &str) -> PathBuf {
        ...
        fs::write(bundle.join("Contents/MacOS/zeus"), marker).expect("binary");
        bundle
    }
    ...
    fn the_helper_actually_replaces_the_bundle() {
        ...
        let target = fake_bundle(directory.path(), "zeus.app", "old");
        ...
        assert_eq!(
            fs::read_to_string(target.join("Contents/MacOS/zeus")).expect("read"),
            "new"
        );
        assert!(
            !directory.path().join("zeus.app.zeus-previous").exists(),
    ...
    fn the_helper_restores_the_old_bundle_when_the_unpack_fails() {
        ...
        assert!(!directory.path().join("zeus.app.zeus-previous").exists());
    }
    ...
    fn the_helper_leaves_the_app_alone_when_the_process_never_exits() {
        ...
        assert_eq!(
            fs::read_to_string(target.join("Contents/MacOS/zeus")).expect("read"),
            "old",
            "a still-running zeus must never have its bundle swapped"
        );
    }
    ...
    fn unpacking_finds_the_single_app_in_an_archive() {
        ...
        fs::create_dir_all(source.join("zeus.app/Contents/MacOS")).expect("bundle layout");
        fs::write(source.join("zeus.app/Contents/MacOS/zeus"), b"binary").expect("binary");

        let archive = directory.path().join("zeus.zip");
        let status = Command::new("/usr/bin/ditto")
            .arg("-c")
            .arg("-k")
            .arg("--keepParent")
            .arg(source.join("zeus.app"))
            .arg(&archive)
            .status()
            .expect("ditto runs");
        ...
        assert_eq!(unpacked.file_name().unwrap(), "zeus.app");
        assert!(unpacked.join("Contents/MacOS/zeus").exists());
    }
