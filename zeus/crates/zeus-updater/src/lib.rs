//! zeus's self-updater.
//!
//! Replaces what Sparkle does for the Swift app, without Sparkle: zeus is a
...
//! password-gated Cloudflare Worker — but zeus reads a JSON feed under
//! `/zeus/` and trusts Apple's Developer ID + notarization rather than a
...
pub const RELEASES_HOST: &str = "zeus-releases.nasrul2001.workers.dev";
pub const DEFAULT_FEED_URL: &str =
    "https://zeus-releases.nasrul2001.workers.dev/zeus/appcast.json";
...
pub const DEFAULT_BASIC_AUTH: &str = "zeus:conduct-crimson-otter-42";
...
pub const ALLOW_UNSIGNED_ENV: &str = "ZEUS_UPDATER_ALLOW_UNSIGNED";
/// Overrides the feed URL, for staging a release before it goes live.
pub const FEED_URL_ENV: &str = "ZEUS_UPDATE_FEED";
...
impl UpdaterConfig {
    ...
    pub fn for_running_app(current_version: &str) -> Result<Self> {
        let bundle = bundle::running_bundle().ok_or_else(|| {
            UpdateError::NotUpdatable("zeus is not running from an app bundle".to_owned())
        })?;
        ...
        Ok(Self {
            ...
            bundle,
            cache_dir: home.join("Library/Caches/zeus/updates"),
            installed_signature,
        })
    }
}
...
impl Updater {
    ...
    pub fn download(&self, release: &Release, on_progress: impl FnMut(f32)) -> Result<PathBuf> {
        ...
        let archive = directory.join("zeus.zip");
        self.http
    ...
    pub fn stage(&self, release: &Release, archive: &Path) -> Result<StagedUpdate> {
        ...
        // The signature check would pass — it is a real zeus build — so the
...
mod tests {
    ...
    fn config() -> UpdaterConfig {
        UpdaterConfig {
            ...
            bundle: PathBuf::from("/Applications/zeus.app"),
            cache_dir: PathBuf::from("/tmp/zeus-updates"),
    ...
    /// `cargo test -p zeus-updater -- --ignored`.
    #[test]
    ...
    fn staging_directories_are_named_by_normalized_version() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/tmp/zeus-updates/0.5.0")
        );
    }
    ...
    fn a_feed_version_that_is_not_a_version_cannot_escape_the_cache_directory() {
        ...
        assert_eq!(
            updater.release_dir(&release),
            PathBuf::from("/tmp/zeus-updates/pending")
        );
    }
