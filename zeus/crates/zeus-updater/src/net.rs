...
//! machine zeus supports. Two requests per update cycle is not a workload that
...
impl Http {
    ...
    fn config(&self, url: &str, output: Option<&Path>, timeout_seconds: u32) -> String {
        ...
        config.push_str(&format!("user-agent = \"zeus-updater/{}\"\n", crate::AGENT));
...
/// decides whether the bytes are really a zeus build.
...
mod tests {
    ...
    const HOST: &str = "zeus-releases.nasrul2001.workers.dev";

    #[test]
    fn accepts_a_release_url_on_the_pinned_host() {
        assert!(
            validated_download_url(
                "https://zeus-releases.nasrul2001.workers.dev/zeus/zeus-0.2.0.zip",
                HOST
            )
            .is_ok()
        );
    ...
    fn rejects_other_hosts_and_schemes() {
        assert!(validated_download_url("https://evil.test/zeus.zip", HOST).is_err());
        assert!(
            validated_download_url(
                "http://zeus-releases.nasrul2001.workers.dev/zeus.zip",
                HOST
            )
            .is_err()
        );
        assert!(validated_download_url("file:///tmp/zeus.zip", HOST).is_err());
    }
    ...
    fn rejects_userinfo_that_would_fake_the_host() {
        assert!(
            validated_download_url(
                "https://zeus-releases.nasrul2001.workers.dev@evil.test/zeus.zip",
                HOST
            )
            .is_err()
        );
    ...
    fn rejects_urls_that_could_inject_curl_options() {
        assert!(
            validated_download_url(
                "https://zeus-releases.nasrul2001.workers.dev/a\"\nupload-file = \"/etc/passwd",
                HOST
            )
            .is_err()
        );
        assert!(
            validated_download_url("https://zeus-releases.nasrul2001.workers.dev/a\\b", HOST)
                .is_err()
        );
    }
    ...
    fn config_quotes_credentials_and_never_puts_them_in_argv() {
        let http = Http::new(Some("zeus:hunter2".to_owned()));
        let config = http.config("https://example.test/a.json", None, 20);
        assert!(config.contains("user = \"zeus:hunter2\"\n"));
    ...
    fn matching_checksum_passes_and_a_wrong_one_fails() {
        ...
        fs::write(&path, b"zeus").expect("write payload");
        // echo -n zeus | shasum -a 256
