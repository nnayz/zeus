...
source_app="${ZEUS_APP_PATH:-${workspace_dir}/dist/zeus.app}"
install_dir="${HOME}/Applications"
installed_app="${install_dir}/zeus.app"
...
if pgrep -x zeus >/dev/null 2>&1; then
    ...
    for _ in $(seq 1 20); do
        pgrep -x zeus >/dev/null 2>&1 || break
        sleep 0.2
    done
fi
rm -rf "${installed_app}"
...
cli_source="${installed_app}/Contents/Resources/bin/zeus"
cli_dir="${ZEUS_CLI_DIR:-${HOME}/.local/bin}"
if [[ -x "${cli_source}" ]]; then
    mkdir -p "${cli_dir}"
    ln -sf "${cli_source}" "${cli_dir}/zeus"
    echo "Linked ${cli_dir}/zeus -> ${cli_source}"
    case ":${PATH}:" in
        *":${cli_dir}:"*) ;;
        *) echo "note: ${cli_dir} is not on PATH; add it to use \`zeus\` directly" >&2 ;;
    esac
else
