...
# CPU at all.
#
# The probe boots the PACKAGED zeusd-rs against a private App Support in a
...
# regression.
#
# Usage: latency-probe.sh --app <zeus.app>
# Env: ZEUS_LATENCY_MEDIAN_MS (default 75), ZEUS_LATENCY_P90_MS (default 150),
#      ZEUS_LATENCY_SAMPLES (default 10)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --app)
            ...
            ;;
        *)
            echo "usage: latency-probe.sh --app <zeus.app>" >&2
            exit 2
            ;;
    esac
done
[[ -d "$app" ]] || {
    echo "error: --app <zeus.app> is required" >&2
    exit 2
}

daemon="$app/Contents/Resources/bin/zeusd-rs"
[[ -x "$daemon" ]] || {
    echo "error: $daemon missing; package first" >&2
    exit 2
}

support="$(mktemp -d /tmp/zeus-latency.XXXXXX)"
daemon_pid=""
cleanup() {
...
ZEUS_APP_SUPPORT="$support" "$daemon" >"$support/boot.log" 2>&1 &
daemon_pid=$!
...
ZEUS_LATENCY_SOCK="$support/daemon.sock" \
ZEUS_LATENCY_MEDIAN_MS="${ZEUS_LATENCY_MEDIAN_MS:-75}" \
ZEUS_LATENCY_P90_MS="${ZEUS_LATENCY_P90_MS:-150}" \
ZEUS_LATENCY_SAMPLES="${ZEUS_LATENCY_SAMPLES:-10}" \
python3 - <<'PROBE'
import json
import os
import socket
...
sock.connect(os.environ["ZEUS_LATENCY_SOCK"])
sock.settimeout(10)
...
samples = []
for index in range(int(os.environ["ZEUS_LATENCY_SAMPLES"])):
...
budget_median = float(os.environ["ZEUS_LATENCY_MEDIAN_MS"])
budget_p90 = float(os.environ["ZEUS_LATENCY_P90_MS"])
print(
