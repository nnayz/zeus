#!/usr/bin/env bash
#
# Packaged-release memory/idle-CPU gate for zeus.app.
#
# The probe launches the bundle executable directly so `$!` is the exact
# process it owns. It never searches for or signals somebody else's Zeus.
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
workspace_dir="$(cd "${script_dir}/.." && pwd)"
app_path="${ZEUS_PERF_APP:-${workspace_dir}/dist/zeus.app}"
scenario="all"
live_daemon=0

usage() {
    cat <<'EOF'
usage: zeus/scripts/perf-gate.sh [--app PATH] [--scenario normal|large|all] [--live-daemon]
...
Environment budgets/tuning:
  ZEUS_PERF_NORMAL_MAX_MB       normal-window footprint ceiling (default 80)
  ZEUS_PERF_LARGE_MAX_MB        1800x1100 footprint ceiling (default 140)
  ZEUS_PERF_IDLE_AVG_CPU        mean idle CPU percent ceiling (default 0.75)
  ZEUS_PERF_IDLE_PEAK_CPU       peak idle CPU percent ceiling (default 1.0)
  ZEUS_PERF_SETTLE_SECONDS      wait before measuring (default 30)
  ZEUS_PERF_CPU_SAMPLES         one-second top samples, including warmup (default 7)
EOF
}
...
executable="${app_path}/Contents/MacOS/zeus"
...
normal_max_mb="${ZEUS_PERF_NORMAL_MAX_MB:-80}"
large_max_mb="${ZEUS_PERF_LARGE_MAX_MB:-140}"
idle_avg_cpu="${ZEUS_PERF_IDLE_AVG_CPU:-0.75}"
idle_peak_cpu="${ZEUS_PERF_IDLE_PEAK_CPU:-1.0}"
...
settle_seconds="${ZEUS_PERF_SETTLE_SECONDS:-30}"
cpu_samples="${ZEUS_PERF_CPU_SAMPLES:-7}"

probe_tmp="$(mktemp -d "${TMPDIR:-/tmp}/zeus-perf.XXXXXX")"
owned_pid=""
owned_start=""
...
run_scenario() {
    ...
    if [[ "$live_daemon" != "1" ]]; then
        launch_environment+=(
            "ZEUS_SIDEBAR_PREVIEW=1"
            "ZEUS_SIDEBAR_SCENARIO=stress"
        )
    fi
    ...
    if [[ "$large_switch" == "1" ]]; then
        launch_environment+=("ZEUS_PERF_LARGE_WINDOW=1")
    fi

    echo "==> Probing packaged zeus: $name"
    ...
    if [[ -z "$owned_start" ]]; then
        echo "error: packaged zeus exited during launch; log follows" >&2
        sed -n '1,160p' "$log" >&2
        return 1
    fi
    ...
    if [[ "$(basename "$command")" != "zeus" ]]; then
        echo "error: PID $owned_pid is not the launched zeus executable ($command)" >&2
        return 1
    fi
    echo "    PID $owned_pid; settling ${settle_seconds}s"
    sleep "$settle_seconds"
    if ! same_owned_process; then
        echo "error: packaged zeus PID $owned_pid exited before measurement" >&2
        sed -n '1,160p' "$log" >&2
        return 1
    fi
...
echo "PASS: packaged zeus is within memory and idle-CPU budgets"
