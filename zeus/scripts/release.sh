#!/bin/bash
# Build, sign, notarize, and publish a zeus release.
#
# Usage: zeus/scripts/release.sh <version>        e.g. zeus/scripts/release.sh 0.2.0
#
# Env overrides:
...
#   RELEASES_DIR        path to the zeus-releases repo (default: ../../zeus-releases)
...
#   zeus-<version>-universal.dmg  what people download by hand
#   zeus-<version>-universal.zip  what the in-app updater fetches
# and rewrites zeus/appcast.json, the feed the updater reads. See
# zeus/UPDATING.md for the trust model and one-time setup.
set -euo pipefail

if [ $# -lt 1 ]; then
    echo "usage: zeus/scripts/release.sh <version>   (e.g. 0.2.0)" >&2
    exit 2
fi
VERSION="$1"
...
DIST="$WORKSPACE/dist"
APP="$DIST/zeus.app"
DMG="$DIST/zeus-$VERSION-universal.dmg"
...
RELEASES_DIR="${RELEASES_DIR:-$ROOT/../zeus-releases}"
RELEASES_HOST="https://zeus-releases.nasrul2001.workers.dev"
# Everything zeus publishes lives under /zeus/ so it never collides with the
# Swift app's DMGs and appcast.xml in the same bucket.
PUBLIC_DIR="$RELEASES_DIR/public/zeus"
...
error: no "Developer ID Application" signing identity found.
  ...
  → "Developer ID Application", then re-run. Or set ZEUS_SIGN_IDENTITY.
  See zeus/UPDATING.md.
EOF
    exit 1
fi

echo "==> Releasing zeus $VERSION"
echo "    Sign identity : $ZEUS_SIGN_IDENTITY"
...
if [ "$CURRENT" != "$VERSION" ]; then
    ...
    echo "==> Bumping zeus-app $CURRENT -> $VERSION"
    ...
    git -C "$ROOT" commit -m "zeus: release $VERSION" >/dev/null
...
# It has happened: an x86_64 zeus-term with a NEWER mtime than its source but
...
FIRST_PARTY=(-p zeus-app -p zeus-client -p zeus-proto -p zeus-term -p zeus-ui -p zeus-updater)
...
ZEUS_VERSION="$VERSION" \
ZEUS_SIGN_IDENTITY="$ZEUS_SIGN_IDENTITY" \
ZEUS_CREATE_DMG=1 \
ZEUS_CREATE_ZIP=1 \
...
# It owns and terminates only the two Zeus processes it launches.
...
# Point the site's download button at this release. zeus bundles the daemon and
# is meant to replace Zeus.app, so a zeus release takes over the button that
...
if [ "${SKIP_INDEX:-0}" != "1" ] && [ -f "$INDEX" ]; then
    if grep -q 'id="download"' "$INDEX"; then
        sed -i '' -E \
            "s|(id=\"download\"[^>]*href=\")[^\"]*(\")|\1zeus/zeus-$VERSION-universal.dmg\2|" \
            "$INDEX"
        echo "==> Download button now serves zeus-$VERSION-universal.dmg"
        echo "    (the page still reads \"Zeus\" — rename it there when you're ready)"
    else
        echo "warning: no id=\"download\" anchor in $INDEX; left the button alone" >&2
    fi
fi

NOTES_FILE="$PUBLIC_DIR/zeus-$VERSION.html"
NOTES_URL=""
if [ -f "$NOTES_FILE" ]; then
    NOTES_URL="$RELEASES_HOST/zeus/zeus-$VERSION.html"
else
...
echo "==> Updating $FEED"
VERSION="$VERSION" \
URL="$RELEASES_HOST/zeus/zeus-$VERSION-universal.zip" \
...
for artifact in list(public.glob("zeus-*-universal.dmg")) + list(public.glob("zeus-*-universal.zip")):
    version = artifact.name.removeprefix("zeus-").removesuffix("-universal.dmg").removesuffix("-universal.zip")
...
git -C "$RELEASES_DIR" commit -m "zeus release $VERSION" || echo "    (nothing to commit)"
...
============================================================
  zeus $VERSION released
============================================================
  DMG        : $PUBLIC_DIR/zeus-$VERSION-universal.dmg
  Update zip : $PUBLIC_DIR/zeus-$VERSION-universal.zip
  sha256     : $SHA256
  Feed       : $RELEASES_HOST/zeus/appcast.json

  Next steps:
    1. Push the releases repo: git -C "$RELEASES_DIR" push
    2. Push + tag the source:  git -C "$ROOT" push && git tag zeus-v$VERSION && git push origin zeus-v$VERSION
    3. Confirm an old build updates itself (zeus/UPDATING.md → "Verifying a release")
============================================================
EOF
