...
# URL. See zeus/UPDATING.md for the trust model and one-time setup.
set -euo pipefail
...
echo "==> Writing $FEED"
VERSION="$VERSION" \
URL="$BASE_URL/zeus-$VERSION-universal.zip" \
...
**Install:** download the DMG below, open it, drag zeus to Applications.
Universal (Apple silicon and Intel), signed and notarized, so it opens without
a Gatekeeper prompt.

zeus updates itself from here — \`appcast.json\` is the feed it reads.
NOTES
...
CASK_FILE="$TAP_DIR/Casks/zeus.rb"
if [ "${SKIP_CASK:-0}" = "1" ]; then
    ...
    if git -C "$TAP_DIR" diff --quiet -- Casks/zeus.rb; then
        echo "    cask already at $VERSION"
    else
        git -C "$TAP_DIR" add Casks/zeus.rb
        git -C "$TAP_DIR" commit -q -m "zeus $VERSION"
...
============================================================
  ...
  Next steps:
    1. Push the source and tag it:
         git -C "$ROOT" push && git tag zeus-v$VERSION && git push origin zeus-v$VERSION
    2. Confirm an old build updates itself (zeus/UPDATING.md → "Verifying a release")
============================================================
EOF
