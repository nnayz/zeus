...
fixture_root="$(mktemp -d "${TMPDIR:-/tmp}/zeus-github-publish-test.XXXXXX")"
...
zip="${fixture_root}/zeus-0.4.6-universal.zip"
...
printf '%s\n' \
    ...
    '    if [[ " $* " == *" --json assets "* ]]; then' \
    '        case "$*" in' \
    '            *zeus-0.4.6-universal.dmg*) printf "sha256:%s\n" "${TEST_DMG_SHA:?}" ;;' \
    '            *zeus-0.4.6-universal.zip*) printf "sha256:%s\n" "${TEST_ZIP_SHA:?}" ;;' \
