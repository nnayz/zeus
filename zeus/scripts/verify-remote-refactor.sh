...
#
#   zeus/scripts/verify-remote-refactor.sh
...
manifests="crates/zeus-engine/manifests"
rc=0
...
check "session.migrate refuses only without a transport" \
    grep -q 'self.remote.is_none()' crates/zeus-engine/src/control.rs
...
check "checkpoints still persist scrollback" \
    grep -q 'pub fn history_snapshot' crates/zeus-terminal-state/src/lib.rs

check "terminal file/URL references survive" \
    grep -q 'pub fn reference_at' crates/zeus-term/src/element.rs
